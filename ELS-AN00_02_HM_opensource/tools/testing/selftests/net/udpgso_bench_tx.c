...
static char pack_buf[NUM_PKT][ETH_MAX_MTU];
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < sizeof(pack_buf[0]); i++)
		pack_buf[0][i] = 'a' + (i % 26);
	for (i = 1; i < NUM_PKT; i++)
		memcpy(pack_buf[i], pack_buf[0], sizeof(pack_buf[0]));
	...
	do {
		if (cfg_tcp)
			num_sends += send_tcp(fd, pack_buf[i]);
		else if (cfg_segment)
			num_sends += send_udp_segment(fd, pack_buf[i]);
		else if (cfg_sendmmsg)
			num_sends += send_udp_sendmmsg(fd, pack_buf[i]);
		else
			num_sends += send_udp(fd, pack_buf[i]);
		num_msgs++;
