	...
	int *valid_add)
{
	...
	if (strnlen(dip_dport->port_key, MPTCP_HW_EXT_PORT_KEY_MAX_LEN) == 0) {
					     ...
					     bool is_add)
{
	...
	case MPTCP_HW_CONF_VALUE_PROXY_INFO:
		if (is_add) {
			if (val->proxy_info.proxy_ip == 0 || val->proxy_info.proxy_port == 0)
				return -EINVAL;
		}
		return 0;
	...
	struct mptcp_hw_ext *config, uint8_t key_type, uint32_t port_to_add)
{
	...
	if (!node || strnlen(port_key, MPTCP_HW_EXT_PORT_KEY_MAX_LEN) == 0 || !value)
		return false;
	...
	for (i = 0; i < MPTCP_PORT_RANGE_NUM_MAX; i++) {
		...
		list_for_each_entry(dport_node, &(node->dport_list), dport_node) {
			if (strncmp(port_key, dport_node->port_key, MPTCP_HW_EXT_PORT_KEY_MAX_LEN) == 0)
				return false;
	...
	struct mptcp_hw_ext_dip_node *node, struct mptcp_hw_ext *config)
{
	...
	if (!node || strnlen(config->conf.dip_dport.port_key, MPTCP_HW_EXT_PORT_KEY_MAX_LEN) == 0)
		return false;

	list_for_each_entry_safe(dport_node, dport_next, &(node->dport_list), dport_node) {
		if (strncmp(dport_node->port_key, config->conf.dip_dport.port_key,
				MPTCP_HW_EXT_PORT_KEY_MAX_LEN) == 0)
			return true;
	}
	...
	__s32 snd_buf, __s32 rcv_buf, const char *if_name)
{
	...
	for (i = 0; i < MAX_SUPPORT_SUBFLOW_NUM; i++) {
		if ((strnlen(if_name, IFNAMSIZ) == strnlen(tp->hw_subflows[i].if_name, IFNAMSIZ))
				&& (strncmp(if_name, tp->hw_subflows[i].if_name, IFNAMSIZ) == 0)) {
			tp->hw_subflows[i].snd_buf = snd_buf;
			tp->hw_subflows[i].rcv_buf = rcv_buf;
			return;
		}

		if (strnlen(tp->hw_subflows[i].if_name, IFNAMSIZ) == 0) {
	...
	uint8_t key_type, uint32_t value_type, uint32_t port_to_add)
{
	...
	if (mptcp_hw_ext_add_new_dport_node(&(dip_node->dport_list), config,
			value_type, port_to_add) != 0) {
		kfree(dip_node);
		return -ENOMEM;
	}
	...
	struct mptcp_hw_ext *config)
{
	...
	list_for_each_entry(pdport_node, dport_list, dport_node) {
		if (strncmp(pdport_node->port_key, dport_key, MPTCP_HW_EXT_PORT_KEY_MAX_LEN) == 0) {
	...
	uint32_t value_type)
{
	...
	case ENABLE_BY_UID_DIP_DPORT_ISP: {
			...
			if (value_type == MPTCP_HW_CONF_VALUE_MPTCP) {
				if (!mptcp_hw_ext_try_to_delete_dport_node(
			...
			if (!mptcp_hw_ext_try_to_delete_config_in_dport_node(
	...
	uint8_t old_key_type, uint8_t new_key_type, bool is_add)
{
	...
	case ENABLE_BY_UID_DIP_DPORT_ISP: {
		...
		if (is_add && value_type == MPTCP_HW_CONF_VALUE_MPTCP) {
	...
	uint8_t key_type, uint32_t value_type)
{
	...
	if (uid_node) {
		...
		if (!mptcp_hw_ext_match_key_type(value_type, uid_node->key_type,
				key_type, true)) {
			pr_err("%d key_type different from last time with uid:%d\n", __LINE__, uid);
			return -EINVAL;
		}

		if (mptcp_hw_ext_update_uid_node_config(uid_node, config, key_type,
				value_type, port_to_add) != 0) {
...
static int mptcp_hw_ext_set_first_path_info(struct mptcp_hw_ext *ext)
{
	...
	if ((len == 0) || (len == IFNAMSIZ) || ext->conf_value.first_path_timeout == 0 || uid <= 0) {
...
EXPORT_SYMBOL(mptcp_hw_ext_cmd_get_sockopt); //lint !e580
...
EXPORT_SYMBOL(mptcp_hw_ext_cmd_set_sockopt); //lint !e580
...
void mptcp_init_sub_sock(struct sock *sk, bool master_sk)
{
	...
	for (i = 0; i < MAX_SUPPORT_SUBFLOW_NUM; i++) {
		if (strnlen(meta_tp->hw_subflows[i].if_name, IFNAMSIZ) == 0) {
			continue;
		}
...
EXPORT_SYMBOL(mptcp_hw_ext_get_port_key); //lint !e580
...
EXPORT_SYMBOL(mptcp_is_subflow_from_iface); //lint !e580
