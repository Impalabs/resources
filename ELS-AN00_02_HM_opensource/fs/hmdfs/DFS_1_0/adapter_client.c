...
static struct metabase *lookup_file(struct file *file)
{
	...
	lookup_result = kzalloc(sizeof(*lookup_result) + path_len, GFP_KERNEL);
	if (!lookup_result)
		goto out_get_result;

	err = dentry_syncer_lookup(req, lookup_result);
	if (err) {
		kfree(lookup_result);
		lookup_result = NULL;
			  ...
			  struct hmdfs_adapter_head *head, void *buf)
{
	...
	if (!msg_info || op >= META_PULL_END || !recv_callbacks[op]) {
		idr_remove(&con->msg_idr, id);
		kfree(msg_info);
		msg_info = NULL;
	} else {
	...
	case READ_RESPONSE:
		...
		if (!cancel_delayed_work(&async_work->d_work))
			...
			recv_callbacks[op](msg_info, buf, len);
		asw_put(async_work);
		break;
	case WRITE_RESPONSE:
		recv_callbacks[op](msg_info, buf, len);
