			    ...
			    struct hmdfs_adapter_head *head, void *buf)
{
	...
	struct file *file = NULL;
	struct adapter_write_request *req_data =
		(struct adapter_write_request *)buf;
	struct adapter_write_data_buffer *write_buf = NULL;
	struct hmdfs_send_data msg;
	__u32 fno = req_data->req_info.fno;
	__u64 local_dno = con->sbi->local_info.iid;
	char *pathname = kmalloc(PATH_MAX, GFP_KERNEL);
	...
	ack_data =
		(struct adapter_write_ack *)((void *)ack_head +
					     sizeof(struct hmdfs_adapter_head));

	for (pos = 0; pos < req_data->size; ) {
		write_buf = (struct adapter_write_data_buffer *)
			    (req_data->data_buf + pos);
		if (write_buf->size == 0)
			break;

		size = write_buf->payload_size;
		offp = vfs_llseek(file, write_buf->offset, SEEK_SET);
		err = kernel_write(file, write_buf->buf, size, &offp);
		pos += write_buf->size;
		if (err > 0)
			real_size += err;
		else
			   ...
			   struct hmdfs_adapter_head *head, void *buf)
{
	...
	struct inode *ino;
	struct hmdfs_send_data msg;
	struct adapter_req_info *read_req = buf;
	__u32 fno = read_req->fno;
	__u64 local_dno = con->sbi->local_info.iid;
	char *pathname = kmalloc(PATH_MAX, GFP_KERNEL);
			 ...
			 struct hmdfs_adapter_head *head, void *buf)
{
	...
	if (head->operations >= META_PULL_END ||
	    !adapter_s_recv_callbacks[head->operations])
		return;

	adapter_s_recv_callbacks[head->operations](con, head, buf);
