/*
 * kcontrol.c -- da combine v5 codec driver
 *
...
#define GPIO_HAC_KCONTROLS \
	SOC_ENUM_EXT("HAC", hac_switch_enum[0], \
		hac_status_get, hac_status_set),\

#endif

#ifdef AUDIO_FACTORY_MODE
static struct da_combine_mbhc *kcontrol_get_mbhc(struct snd_kcontrol *kcontrol)
{
...
{
	int headset_type;
	struct da_combine_mbhc *mbhc = NULL;
	...
	headset_type = da_combine_get_4_pole_headset_type(mbhc);
...
static int set_4_pole_headset_type(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (headset_type)
		...
		da_combine_set_audio_jack_headset(mbhc);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	da_combine_update_bits(codec, S1_IL_PGA_CTRL1_REG, 0xff, gain);
	da_combine_update_bits(codec, S1_IR_PGA_CTRL1_REG, 0xff, gain);
	return 0;
}

#define DA_COMBINE_V5_HIGH_RES_HS_KCONTROLS \
...
#endif
#ifdef CONFIG_HIGH_RESISTANCE_HS_DET
	DA_COMBINE_V5_HIGH_RES_HS_KCONTROLS
#endif
};


int da_combine_v5_add_kcontrol(struct snd_soc_component *codec)
{
	if (codec == NULL) {
		AUDIO_LOGE("parameter is null");
		return -EINVAL;
