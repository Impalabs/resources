...
void blk_scsi_kern_time_stamp_dump(void)
{
	...
	for (i = 0; i < MAX_TIME_RECORD_COUNT; i++)
		pr_err("req_get_done %llu, req_exec_start %llu, req_exec_end %llu, is_current_io %d\n",
...
static void blk_scsi_kern_time_record(enum time_record_type type)
{
	...
	switch (type) {
	case REQ_GET_DONE:
		blk_scsi_cur_time[i].req_get_done = hisi_getcurtime();
		break;
	case REQ_EXEC_START:
		blk_scsi_cur_time[i].req_exec_start = hisi_getcurtime();
		break;
	case REQ_EXEC_END:
		blk_scsi_cur_time[i].req_exec_end = hisi_getcurtime();
					     ...
					     struct sg_io_v4 *hdr)
{
	...
	blk_scsi_kern_time_record(REQ_GET_DONE);
			...
			unsigned long arg, bool need_order)
{
	...
	case SG_IO:
		...
		blk_scsi_kern_time_record(REQ_EXEC_END);
