 ...
 * Description: the tcrypt_mdpp_selfverify.c - execute kenel mdpp sleftest
...
static int do_alg_tests(void)
{
	...
	for (i = 0; i < (int32_t)ARRAY_SIZE(test_algos); i++) {
		int ret;
...
static long tcrypt_base_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case TCRYPT_READ_RESULT:
		if (copy_to_user((int *)(uintptr_t)arg, &g_latest_result,
...
/* fops define */
static const struct file_operations g_tcrypt_fops = {
	.owner	 = THIS_MODULE,
