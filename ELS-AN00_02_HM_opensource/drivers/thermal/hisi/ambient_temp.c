...
static int hisi_get_ambient_temp(struct thermal_zone_device *thermal, int *temp)
{
	struct hisi_ambient_t *hisi_ambient = thermal->devdata;

	if (hisi_ambient == NULL || temp == NULL)
		return -EINVAL;
...
static int hisi_ambient_sensors_dts_parse(struct device_node *np,
					  struct hisi_ambient_t *hisi_ambient)
{
	...
	for_each_child_of_node(np, child) {
		ambient_sensor = (struct hisi_ambient_sensor_t *)(uintptr_t)
				 ((u64)(uintptr_t)hisi_ambient +
				  sizeof(struct hisi_ambient_t) + (u64)((long)i) *
				  (sizeof(struct hisi_ambient_sensor_t)));
		ret = of_property_read_string(child, "type", &ptr_type);
		if (ret != 0) {
			pr_err("%s type read err\n", __func__);
			return -EINVAL;
...
static int hisi_ambient_probe(struct platform_device *pdev)
{
	...
	hisi_ambient = kzalloc(sizeof(struct hisi_ambient_t) +
			       (u64)((long)sensor_count) *
			       (sizeof(struct hisi_ambient_sensor_t)),
			       GFP_KERNEL);
	if (hisi_ambient == NULL) {
		ret = -ENOMEM;
	...
	ret = hisi_ambient_sensors_dts_parse(np, hisi_ambient);
	if (ret != 0)
		goto free_mem;
	...
	hisi_ambient->tz_dev =
		thermal_zone_device_register(dev_node->name, 0, 0, hisi_ambient,
...
int hisi_ambient_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (hisi_ambient != NULL) {
		...
		if (hisi_ambient->start_cc == -1) {
			...
			do_gettimeofday(&hisi_ambient->now);
			delta_time = hisi_ambient->now.tv_sec -
				     hisi_ambient->last_time.tv_sec;
			if (delta_time == 0)
				return 0;
			/* unit modify from uah to mah */
			delta_cc = abs(now_cc - hisi_ambient->start_cc) / 1000;
...
int hisi_ambient_resume(struct platform_device *pdev)
{
	...
	if (delta_time >= hisi_ambient->interval &&
		...
		pr_info("%s time pass %ld > interval %d s , "
			...
			__func__, delta_time,  hisi_ambient->interval,
			now_cc, hisi_ambient->start_cc, avg_current);

		for (i = 0; i < hisi_ambient->sensor_count ; i++) {
			ambient_sensor = (struct hisi_ambient_sensor_t *)(uintptr_t)
					  ((u64)(uintptr_t)hisi_ambient +
					  sizeof(struct hisi_ambient_t) +
					  (u64)((long)i) *
					  (sizeof(struct hisi_ambient_sensor_t)));
		...
		pr_err("%s ambient temp: %d at UTC time: %d-%d-%d %d:%d:%d.\n",
		       __func__, hisi_ambient->temp, tm.tm_year + 1900,
