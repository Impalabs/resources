...
#ifdef CONFIG_BALT_IP_PLATFORM
void warn_on_ppll2_disable(void)
{
	if (!IS_ERR_OR_NULL(dss_clk) && dss_clk->core) {
		/* if clk_edc0 is enable and rate is 480000000, warnon */
		WARN_ON(dss_clk->core->enable_count > 0 && dss_clk->core->rate == FIX_DSS_RATE);
	} else {
		pr_err("%s get clk_edc0 fail\n", __func__);
	}
}
