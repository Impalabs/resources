...
static ssize_t lcd_kit_lcd_cabc_mode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret = LCD_KIT_OK;
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
	...
	mutex_lock(&COMMON_LOCK->sysfs_lock.cabc_mode_lock);
	if (sysfs_ops->lcd_cabc_mode_show)
		ret = sysfs_ops->lcd_cabc_mode_show(dev, attr, buf);
	mutex_unlock(&COMMON_LOCK->sysfs_lock.cabc_mode_lock);
	return ret;
}

static ssize_t lcd_kit_lcd_cabc_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct lcd_kit_sysfs_ops *sysfs_ops = NULL;
	sysfs_ops = lcd_kit_get_sysfs_ops();
	...
	mutex_lock(&COMMON_LOCK->sysfs_lock.cabc_mode_lock);
	if (sysfs_ops->lcd_cabc_mode_store)
		sysfs_ops->lcd_cabc_mode_store(dev, attr, buf, count);
	mutex_unlock(&COMMON_LOCK->sysfs_lock.cabc_mode_lock);
	return count;
}
...
static DEVICE_ATTR(lcd_cabc_mode, 0644, lcd_kit_lcd_cabc_mode_show,
	lcd_kit_lcd_cabc_mode_store);
