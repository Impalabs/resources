...
static void dump_fingers_info_debug(struct ts_finger *fingers)
{
	int i = 0;
	...
	for (; i < TS_MAX_FINGER; i++) {
		if (fingers[i].status)
			TS_LOG_INFO(
				"%s:id=%d status:%d pressure=%d or=%d maj=%d min=%d wx=%d wy=%d ewx=%d ewy=%d xer=%d yer=%d\n",
	...
	for (i = 0; (i < algo_size) && (order & BIT_MASK(i)); i++) {
		...
		list_for_each_entry(algo,
			...
			if (algo->algo_index == i) {
				...
				memcpy(in_finger, out_finger, sizeof(*in_finger));
				break;
			}
		}
	}
out:
	...
	if (g_ts_kit_platform_data.aft_param.aft_enable_flag) {
		if (atomic_read(&g_ts_kit_platform_data.fingers_waitq_flag) ==
			AFT_WAITQ_WAIT) {
			memcpy(&g_ts_kit_platform_data.fingers_send_aft_info,
				&out_cmd->cmd_param.pub_params.report_info,
				sizeof(struct ts_fingers));
			dump_fingers_info_debug(g_ts_kit_platform_data.fingers_send_aft_info.fingers);
			atomic_set(&g_ts_kit_platform_data.fingers_waitq_flag,
				AFT_WAITQ_WAKEUP);
			up(&g_ts_kit_platform_data.fingers_aft_send);
			ts_work_after_input(); /* read roi data for some ic */
			out_cmd->command = TS_INVAILD_CMD;
		} else if (atomic_read(&g_ts_kit_platform_data.fingers_waitq_flag)
			!= AFT_WAITQ_IDLE) {
			up(&g_ts_kit_platform_data.fingers_aft_send);
			TS_LOG_DEBUG("%s hal aglo process too slow\n", __func__);
		} else {
			TS_LOG_DEBUG("[MUTI_AFT] %s no wait\n", __func__);
		}
	}
}
	...
	struct input_dev *input_dev)
{
	...
	/*
	 * touchplus(LingXiYiZhi) report,
	 * The difference between ABS_report and touchpls key_report
	...
	/*
	 * touchplus(LingXiYiZhi) report,
	 * store touchpls key data(finger->special_button_key)
	 * when special_button_flag report touchpls key DOWN,
	 * store current touchpls key till the key report UP,
	 * then other keys will not report
	 */
...
void ts_report_input(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	...
	TS_LOG_DEBUG("%s done, finger_num = %d\n", __func__, finger_num);
	...
	struct ts_cmd_node *out_cmd)
{
	...
	if (g_ts_kit_platform_data.chip_data->easy_wakeup_info.sleep_mode ==
		...
		switch (pm_type) {
		case TS_BEFORE_SUSPEND: /* do something before suspend */
			if (g_ts_kit_platform_data.watch_tp_recovery &&
				(atomic_read(&g_ts_kit_platform_data.state) == TS_GOTO_SLEEP)) {
				TS_LOG_INFO("%s:state already in TS_GOTO_SLEEP,skip\n", __func__);
				return error;
			}
			if (!(g_ts_kit_platform_data.chip_data->is_parade_solution == true)) {
				ts_stop_wd_timer(&g_ts_kit_platform_data);
			...
			if (dev->ops->chip_before_suspend)
				error = dev->ops->chip_before_suspend();
			if (g_ts_kit_platform_data.watch_tp_recovery) {
				atomic_set(&g_ts_kit_platform_data.state, TS_GOTO_SLEEP);
			}
			break;
		case TS_SUSPEND_DEVICE: /* switch to easy-wakeup mode, and enable interrupts */
			if (g_ts_kit_platform_data.watch_tp_recovery &&
				...
				TS_LOG_INFO("%s:state already in TS_WORK_IN_SLEEP,skip\n", __func__);
				return error;
			}
		...
		case TS_RESUME_DEVICE: /* exit easy-wakeup mode and restore sth */
			if (g_ts_kit_platform_data.watch_tp_recovery &&
				(atomic_read(&g_ts_kit_platform_data.state) == TS_GOTO_WORK)) {
				TS_LOG_INFO("%s:state already in TS_GOTO_WORK,skip\n", __func__);
				return error;
			}
			if (!(g_ts_kit_platform_data.chip_data->is_parade_solution == true))
				disable_irq(irq_id);
			if (dev->ops->chip_resume)
				error = dev->ops->chip_resume();
			if (g_ts_kit_platform_data.watch_tp_recovery) {
				atomic_set(&g_ts_kit_platform_data.state, TS_GOTO_WORK);
			}
			break;
		case TS_AFTER_RESUME: /* do nothing */
			if (g_ts_kit_platform_data.watch_tp_recovery &&
				(atomic_read(&g_ts_kit_platform_data.state) == TS_WORK)) {
				TS_LOG_INFO("%s:state already in TS_WORK,skip\n", __func__);
				return error;
			}
			if (dev->ops->chip_after_resume)
				error = dev->ops->chip_after_resume((void *)&g_ts_kit_platform_data.feature_info);
...
int ts_touch_window(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	...
	g_ts_kit_platform_data.feature_info.window_info.window_enable =
		info->window_enable;
	g_ts_kit_platform_data.feature_info.window_info.top_left_x0 =
		info->top_left_x0;
	g_ts_kit_platform_data.feature_info.window_info.top_left_y0 =
		info->top_left_y0;
	g_ts_kit_platform_data.feature_info.window_info.bottom_right_x1 =
		info->bottom_right_x1;
	g_ts_kit_platform_data.feature_info.window_info.bottom_right_y1 =
...
bool ts_cmd_need_process(struct ts_cmd_node *cmd)
{
	...
	if (unlikely((atomic_read(&g_ts_kit_platform_data.state) == TS_SLEEP) ||
		...
		if (atomic_read(&g_ts_kit_platform_data.state) == TS_SLEEP) {
			...
			case TS_INT_ERR_OCCUR:
				if (g_ts_kit_platform_data.chip_data->is_parade_solution) {
					if (g_ts_kit_platform_data.chip_data->isbootupdate_finish == false ||
						g_ts_kit_platform_data.chip_data->sleep_in_mode == 0)
						/* deep sleep mode need process irq when suspend/resume */
						is_need_process = true;
					else
						is_need_process = false;
				} else {
			...
			case TS_POWER_CONTROL:
				if ((pm_type != TS_RESUME_DEVICE) && (pm_type != TS_AFTER_RESUME) &&
					(pm_type != TS_I2C_TO_AP) && (pm_type != TS_I2C_TO_MCU))
					is_need_process = false;
				break;
			case TS_TOUCH_WINDOW:
				is_need_process = true;
...
int ts_kit_power_control_notify(enum ts_pm_type pm_type, int timeout)
{
	...
	if (g_ts_kit_platform_data.chip_data->report_tui_enable && (pm_type == TS_BEFORE_SUSPEND)) {
		g_ts_kit_platform_data.chip_data->tui_set_flag |= 0x1;
		TS_LOG_INFO("TUI is working, later do before suspend\n");
		return NO_ERR;
	}

	if (g_ts_kit_platform_data.chip_data->report_tui_enable && (pm_type == TS_SUSPEND_DEVICE)) {
	...
	struct ts_cmd_node *out_cmd, struct ts_cmd_sync *sync)
{
	...
	if (dev->ops->chip_get_rawdata)
		error = dev->ops->chip_get_rawdata((struct ts_rawdata_info *)info,
			out_cmd);
...
void ts_special_hardware_test_switch(struct ts_cmd_node *in_cmd,
	struct ts_cmd_node *out_cmd)
{
	struct ts_kit_device_data *dev = g_ts_kit_platform_data.chip_data;
	struct ts_special_hardware_test_info *info =
		(struct ts_special_hardware_test_info *)in_cmd->cmd_param.prv_params;
