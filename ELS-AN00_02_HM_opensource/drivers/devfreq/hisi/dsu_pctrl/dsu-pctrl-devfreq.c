...
noinline u64 atfd_dsu_pctrl_smc(u64 _function_id,
...
static void set_portion_ctrl(u64 val)
{
	atfd_dsu_pctrl_smc((u64)DSU_PCTRL_SVC,
			   (u64)CMD_SET_PORTION_CTRL,
			   (u64)val,
			   (u64)0);
}

#ifdef CONFIG_DSU_PCTRL_DEBUG
static ssize_t store_downsize_ratio(struct device *dev,
...
static ssize_t show_downsize_ratio(struct device *dev,
				   struct device_attribute *attr,
				   char *buf)
{
	struct devfreq *devfreq = to_devfreq(dev);
...
static ssize_t store_upsize_ratio(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf,
				  size_t count)
{
...
static ssize_t show_upsize_ratio(struct device *dev,
				 struct device_attribute *attr,
				 char *buf)
{
	struct devfreq *devfreq = to_devfreq(dev);
...
static ssize_t store_static_leakage(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf,
				    size_t count)
{
	...
	if (value > 0) {
		...
		dsu->downsize_threshold = dsu->downsize_ratio;
		dsu->downsize_threshold = (dsu->downsize_threshold *
					   ROUNDING_INTEGER_MUL) / PERCENT_MAX;
		dsu->downsize_threshold *= dsu->static_leakage;
		dsu->downsize_threshold /= dsu->dram_energy_per_mb;
		...
		dsu->upsize_threshold = PERCENT_MAX - dsu->upsize_ratio;
		dsu->upsize_threshold = (dsu->upsize_threshold *
					 ROUNDING_INTEGER_MUL) / PERCENT_MAX;
		dsu->upsize_threshold *= dsu->static_leakage;
		dsu->upsize_threshold /= dsu->dram_energy_per_mb;
...
static ssize_t show_static_leakage(struct device *dev,
				   struct device_attribute *attr,
				   char *buf)
{
	struct devfreq *devfreq = to_devfreq(dev);
	...
	return snprintf_s(buf, PAGE_SIZE,
			  PAGE_SIZE - 1, "%u\n", dsu->static_leakage_per_mb);
}

static ssize_t store_dram_energy(struct device *dev,
				 struct device_attribute *attr,
				 const char *buf,
				 size_t count)
{
...
static ssize_t show_dram_energy(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	struct devfreq *devfreq = to_devfreq(dev);
...
static ssize_t show_max_threshold(struct device *dev,
				  struct device_attribute *attr,
				  char *buf)
{
	struct devfreq *devfreq = to_devfreq(dev);
				   ...
				   size_t count)
{
	...
	if (dsu->up_polling_ms && value > 0 && value <= 10) {
		...
		dsu->down_polling_ms = dsu->down_interval *
...
static void set_pwr_ctrl(u64 val)
{
	atfd_dsu_pctrl_smc((u64)DSU_PCTRL_SVC,
			   (u64)CMD_SET_PWR_CTRL,
			   (u64)val,
			   (u64)0);
}
				 ...
				 char *buf)
{
	...
	/* CLUSTERPWRSTAT_EL1 */
	asm volatile("mrs %0, s3_0_c15_c3_7" : "=r" (val));
...
static void dsu_pctrl_stop_hwmon(struct perf_hwmon *hw)
{
	...
	cpuhp_remove_state_nocalls(CPUHP_AP_DSU_PCTRL_DEVFREQ_NOTIFY_PREPARE);
}
...
static int dsu_pctrl_start_hwmon(struct perf_hwmon *hw)
{
	...
	cpuhp_setup_state_nocalls(CPUHP_AP_DSU_PCTRL_DEVFREQ_NOTIFY_PREPARE,
				  "dsu-pctrl-devfreq:online",
...
static void decide_enable_acp(struct dsu_pctrl *dsu, unsigned long portions)
{
	if (g_req_pending_flag && portions <= dsu->non_share_size) {
		if (!g_acp_enable_flag) {
			g_acp_enable_flag = true;
			l3_cache_enable_acp(dsu->l3share_id, dsu->l3share_size);
			trace_dsu_pctrl_target_acp("enable acp", dsu->l3share_id);
		}
	} else if (g_acp_enable_flag && portions > dsu->non_share_size) {
					 ...
					 unsigned long *portions)
{
	...
	if (!g_extension_enable_status) {
		decide_enable_acp(dsu, *portions);
		dsu->cur_num_portions = *portions;
		portion_active = (1UL << dsu->cur_num_portions) - 1;
		set_portion_ctrl(portion_active);
	} else {
		dsu->cur_num_portions = data->portion_max;
		portion_active = (1UL << dsu->cur_num_portions) - 1;
	}

	if (dsu->cur_num_portions == data->portion_max)
		g_dpc_extension_flag = true;
	else
		g_dpc_extension_flag = false;
	spin_unlock(&g_l3extension_lock);
#else /* CONFIG_L3CACHE_EXTENSION_DYNAMIC */
	decide_enable_acp(dsu, *portions);
	dsu->cur_num_portions = *portions;
	portion_active = (1UL << dsu->cur_num_portions) - 1;
	set_portion_ctrl(portion_active);
...
#else /* CONFIG_L3CACHE_SHARE_PERF */
	dsu->cur_num_portions = *portions;
	portion_active = (1UL << dsu->cur_num_portions) - 1;
	set_portion_ctrl(portion_active);
#endif /* CONFIG_L3CACHE_SHARE_PERF */

	trace_dsu_pctrl_set_active_portions(dsu->cur_num_portions,
					    portion_active);
	trace_clock_set_rate("dsu_pctrl", dsu->cur_num_portions,
...
static int dsu_pctrl_up_size_check(struct dsu_pctrl *dsu)
{
	 ...
	 */
	if (dsu->cur_num_portions >= data->portion_max &&
	    (dsu->alg.usec_down < dsu->down_polling_ms * 1000))
		goto cleanup;
...
static unsigned long dsu_pctrl_calc_next_portions(struct dsu_pctrl *dsu)
{
	...
	if (dsu->alg.usec_down >= dsu->down_polling_ms * 1000) {
...
static int dsu_pctrl_parse_dt(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(node, "downsize-portion-threshold",
				   &dsu->downsize_ratio);
	if (ret != 0)
		dsu->downsize_ratio = DOWNSIZE_PORTION_THRESHOLD;
	dev_info(&pdev->dev, "downsize_ratio = %d\n", dsu->downsize_ratio);

	ret = of_property_read_u32(node, "upsize-portion-threshold",
				   &dsu->upsize_ratio);
	if (ret != 0)
			    ...
			    unsigned long mode, void *data)
{
	...
	case L3SHARE_MON_START:
		...
		dsu->non_share_size = MAX_PORTIONS_NUM - dsu->l3share_size;
		/*
		 * if DSS request full size of L3Cache,
		 * group0 is shareable between acpu and DSS
		 */
		if (dsu->non_share_size >= MAX_PORTIONS_NUM ||
		    dsu->non_share_size == 0)
			dsu->non_share_size = 1;
...
static int dsu_pctrl_devfreq_remove(struct platform_device *pdev)
{
	...
	if (ret != 0) {
		pr_err("%s: failed to set portions: %d.\n", __func__, ret);
		return ret;
	}
