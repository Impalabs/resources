					      ...
					      unsigned int event, void *data)
{
	...
	case DEVFREQ_GOV_LIMITS:
		l3c_devfreq_governor_apply_limits(devfreq,
						  (unsigned long *)data);
		break;
	default:
		break;
				     ...
				     struct device_node *np)
{
	...
	l3c->load_map_len = ((unsigned int)prop->length) /
			    sizeof(*l3c->load_map);
	...
	if (of_property_read_u64_array(np, "load-map", l3c->load_map,
				       l3c->load_map_len) < 0) {
...
#ifdef CONFIG_HW_VOTE_L3C_FREQ
	if (l3c->hv_supported != 0) {
		l3c->l3c_hvdev = hvdev_register(&pdev->dev,
						     "l3-freq", "vote-src-1");
