...
#define FFU_MAX_REQ_SIZE               0x100000 /* ffu modified max_req_size=1M */
	...
	u8 *data, unsigned int buf_bytes)
{
	...
	tmp = card->host->max_req_size;
	pr_err("FFU: default max_req_size = %d\n", tmp);
	card->host->max_req_size = FFU_MAX_REQ_SIZE;
	err = mmc_ffu_write(card, data, cmd->arg, buf_bytes);
	card->host->max_req_size = tmp;
