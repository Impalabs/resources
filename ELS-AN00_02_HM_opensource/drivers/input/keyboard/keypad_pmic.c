...
#define KEYPAD_PMIC_KEY_IRQ_CNT 4
static const char *keypad_pmic_key_irq_table[KEYPAD_PMIC_KEY_IRQ_CNT] = {
	"vol-up-up", "vol-up-down", "vol-down-up", "vol-down-down"
};
#else
#define KEYPAD_PMIC_KEY_IRQ_CNT 2
static const char *keypad_pmic_key_irq_table[KEYPAD_PMIC_KEY_IRQ_CNT] = {
	"vol-up", "vol-down",
};
#endif
...
struct keypad_pmic_key {
	...
	int gpiokey_irq[KEYPAD_PMIC_KEY_IRQ_CNT];
};
...
static unsigned int keypad_pmic_key_read(int gpio_address)
{
	return pmic_read_reg(gpio_address);
}

static void keypad_pmic_key_write(int gpiokey_set, int gpio_address)
{
	pmic_write_reg(gpio_address, gpiokey_set);
}

static int keypad_pmic_key_open(struct input_dev *dev)
{
	return 0;
}

static void keypad_pmic_key_close(struct input_dev *dev)
{
}

static void keypad_pmic_keyup_work(struct work_struct *work)
{
	struct keypad_pmic_key *gpio_key = container_of(
		work, struct keypad_pmic_key, gpio_keyup_work.work);
	unsigned int keyup_value;
	unsigned int rpt_action = GPIO_KEY_RELEASE;

	keyup_value = keypad_pmic_key_read(PMIC_GPIO_KEYUP_DATA);
#if defined(CONFIG_HISI_HI6421V900_PMU) || defined(CONFIG_HISI_HI6555V500_PMU)
	...
	if (key_mask_flag == VOLUME_KEY_ENABLE_MASK) {
		...
		if (rpt_action == GPIO_KEY_PRESS)
			keypad_pmic_call_notifiers(KEYPAD_PMIC_UP_PRESS, NULL);
		else
			keypad_pmic_call_notifiers(KEYPAD_PMIC_UP_RELEASE, NULL);
#ifdef CONFIG_KEY_MASK
	} else {
...
static void keypad_pmic_keydown_work(struct work_struct *work)
{
	struct keypad_pmic_key *gpio_key = container_of(
		work, struct keypad_pmic_key, gpio_keydown_work.work);
	unsigned int keydown_value;
	unsigned int rpt_action = GPIO_KEY_RELEASE;

	keydown_value = keypad_pmic_key_read(PMIC_GPIO_KEYDOWN_DATA);
#if defined(CONFIG_HISI_HI6421V900_PMU) || defined(CONFIG_HISI_HI6555V500_PMU)
	...
	if (key_mask_flag == VOLUME_KEY_ENABLE_MASK) {
		...
		input_sync(gpio_key->input_dev);
		if (rpt_action == GPIO_KEY_PRESS)
			keypad_pmic_call_notifiers(KEYPAD_PMIC_DOWN_PRESS, NULL);
		else
			keypad_pmic_call_notifiers(KEYPAD_PMIC_DOWN_RELEASE, NULL);
#ifdef CONFIG_KEY_MASK
	} else {
		pr_info("[gpiokey]volumn key is disabled %u\n", rpt_action);
	}
...
static void keypad_pmic_keyup_timer(unsigned long data)
{
	unsigned int keyup_value;
	struct keypad_pmic_key *gpio_key =
		(struct keypad_pmic_key *)(uintptr_t)data;
#if defined(CONFIG_HUAWEI_DSM)
...
static void keypad_pmic_keydown_timer(unsigned long data)
{
	u32 keydown_value;
	struct keypad_pmic_key *gpio_key =
		(struct keypad_pmic_key *)(uintptr_t)data;
...
#if defined(CONFIG_HISI_HI6421V900_PMU) || defined(CONFIG_HISI_HI6555V500_PMU)
static void keypad_pmic_v900_clear_int(u32 num)
{
	/* num value is less than 31 */
	keypad_pmic_key_write((1 << num), PMIC_GPIO_KEY_INT);
}

static irqreturn_t keypad_pmic_key_irq_handler(int irq, void *dev_id)
{
	struct keypad_pmic_key *gpio_key =
		(struct keypad_pmic_key *)dev_id;
	u32 key_event;
	...
	if ((irq == gpio_key->gpiokey_irq[0]) ||
		...
		key_event = keypad_pmic_key_read(PMIC_GPIO_KEYUP_DATA);
		...
		if (irq == gpio_key->gpiokey_irq[0])
			keypad_pmic_v900_clear_int(PMIC_KEYPAD_UP_UP_INT);
		else
			keypad_pmic_v900_clear_int(PMIC_KEYPAD_UP_DOWN_INT);
	} else if ((irq == gpio_key->gpiokey_irq[2]) ||
			(irq == gpio_key->gpiokey_irq[3])) {
		key_event = keypad_pmic_key_read(PMIC_GPIO_KEYDOWN_DATA);
		...
		if (irq == gpio_key->gpiokey_irq[2])
			keypad_pmic_v900_clear_int(PMIC_KEYPAD_DOWN_UP_INT);
		else
			keypad_pmic_v900_clear_int(PMIC_KEYPAD_DOWN_DOWN_INT);
	} else {
...
static int keypad_pmic_key_irq_init(struct platform_device *pdev,
	struct keypad_pmic_key *gpio_key)
{
	int i, ret;

	for (i = 0; i < KEYPAD_PMIC_KEY_IRQ_CNT; i++) {
		gpio_key->gpiokey_irq[i] = platform_get_irq_byname(
			pdev, keypad_pmic_key_irq_table[i]);
		if (gpio_key->gpiokey_irq[i] < 0) {
			dev_err(&pdev->dev, "Failed to get %s irq number!\n",
				keypad_pmic_key_irq_table[i]);
			return -ENODEV;
		}

		ret = devm_request_irq(&pdev->dev, gpio_key->gpiokey_irq[i],
			keypad_pmic_key_irq_handler, IRQF_NO_SUSPEND,
			keypad_pmic_key_irq_table[i], gpio_key);
		if (ret) {
...
#if defined(CONFIG_HISI_HI6421V900_PMU) || defined(CONFIG_HISI_HI6555V500_PMU)
static void keypad_pmic_key_reg_init(void)
{
	/* clear interrupt */
	keypad_pmic_key_write(PMIC_KEYUP_INT_CLEAR, PMIC_GPIO_KEY_INT);
}
#else
static void keypad_pmic_key_reg_init(void)
{
	/* 20ms filter */
	keypad_pmic_key_write(KEY_VALUE_HIGH, PMIC_GPIO_KEYUP_DEBSEL);
	keypad_pmic_key_write(KEY_VALUE_HIGH, PMIC_GPIO_KEYDOWN_DEBSEL);
	/* not PD */
	keypad_pmic_key_write(KEY_VALUE_LOW, PMIC_GPIO_KEYUP_PDSEL);
	keypad_pmic_key_write(KEY_VALUE_LOW, PMIC_GPIO_KEYDOWN_PDSEL);
	/* PU */
	keypad_pmic_key_write(KEY_VALUE_HIGH, PMIC_GPIO_KEYUP_PUSEL);
	keypad_pmic_key_write(KEY_VALUE_HIGH, PMIC_GPIO_KEYDOWN_PUSEL);
	/* input */
	keypad_pmic_key_write(KEY_VALUE_LOW, PMIC_GPIO_KEYUP_DIR);
	keypad_pmic_key_write(KEY_VALUE_LOW, PMIC_GPIO_KEYDOWN_DIR);
	/* edge trigger */
	keypad_pmic_key_write(KEY_VALUE_LOW, PMIC_GPIO_KEYUP_IS);
	keypad_pmic_key_write(KEY_VALUE_LOW, PMIC_GPIO_KEYDOWN_IS);
	/* double edge trigger */
	keypad_pmic_key_write(KEY_VALUE_HIGH, PMIC_GPIO_KEYUP_IBE);
	keypad_pmic_key_write(KEY_VALUE_HIGH, PMIC_GPIO_KEYDOWN_IBE);
}
#endif

#ifdef CONFIG_KEY_MASK
static ssize_t keypad_pmic_key_mask_read(struct file *file,
...
static ssize_t keypad_pmic_key_mask_write(struct file *file,
...
static const struct file_operations key_mask_fops = {
	.read = keypad_pmic_key_mask_read,
	.write = keypad_pmic_key_mask_write,
};
#endif

static struct input_dev *keypad_pmic_key_new_input_dev(
	struct platform_device *pdev)
{
	...
	input_dev->open = keypad_pmic_key_open;
	input_dev->close = keypad_pmic_key_close;

	return input_dev;
}

static struct keypad_pmic_key *keypad_pmic_key_new_drvdata(
	struct platform_device *pdev)
{
	struct keypad_pmic_key *gpio_key = NULL;
	struct input_dev *input_dev = NULL;

	gpio_key = devm_kzalloc(
		&pdev->dev, sizeof(struct keypad_pmic_key), GFP_KERNEL);
	if (!gpio_key)
		return NULL;

	input_dev = keypad_pmic_key_new_input_dev(pdev);
...
static void keypad_pmic_key_init_work(struct keypad_pmic_key *gpio_key)
{
	INIT_DELAYED_WORK(
		&(gpio_key->gpio_keyup_work), keypad_pmic_keyup_work);
	INIT_DELAYED_WORK((struct delayed_work *)(uintptr_t)
		(&(gpio_key->gpio_keydown_work)),
		keypad_pmic_keydown_work);
...
static void keypad_pmic_key_setup_timer(struct keypad_pmic_key *gpio_key)
{
	setup_timer(&(gpio_key->key_up_timer), keypad_pmic_keyup_timer,
		(uintptr_t)gpio_key);
	setup_timer(&(gpio_key->key_down_timer), keypad_pmic_keydown_timer,
		(uintptr_t)gpio_key);
...
static int keypad_pmic_key_probe(struct platform_device *pdev)
{
	struct keypad_pmic_key *gpio_key = NULL;
	int err;

	dev_info(&pdev->dev, "keypad pmic key driver probes start!\n");

	gpio_key = keypad_pmic_key_new_drvdata(pdev);
	if (!gpio_key) {
	...
	keypad_pmic_key_init_work(gpio_key);

	keypad_pmic_key_setup_timer(gpio_key);

	keypad_pmic_key_reg_init();
	...
	err = keypad_pmic_key_irq_init(pdev, gpio_key);
	if (err) {
		dev_err(&pdev->dev, "Failed to request irq!\n");
		goto err_request_irq;
	}
	dev_info(&pdev->dev, "keypad pmic key driver probes successfully!\n");
...
static int keypad_pmic_key_remove(struct platform_device *pdev)
{
	struct keypad_pmic_key *gpio_key = platform_get_drvdata(pdev);
	bool ret = true;
...
#ifdef CONFIG_PM
static int keypad_pmic_key_suspend(
...
static int keypad_pmic_key_resume(struct platform_device *pdev)
{
...
static const struct of_device_id keypad_pmic_key_match[] = {
	{
		.compatible = "hisilicon,pmic-gpio-key",
	},
	{},
};
MODULE_DEVICE_TABLE(of, keypad_pmic_key_match);

struct platform_driver keypad_pmic_key_driver = {
	.probe = keypad_pmic_key_probe,
	.remove = keypad_pmic_key_remove,
	.driver = {
		.name = "keypad_pmic_key",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(keypad_pmic_key_match),
	},
#ifdef CONFIG_PM
	.suspend = keypad_pmic_key_suspend,
	.resume = keypad_pmic_key_resume,
#endif
};

module_platform_driver(keypad_pmic_key_driver);

MODULE_DESCRIPTION("keypad pmic platform driver");
