...
static struct directory *create_file(const char *cur_path, unsigned int size)
{
	...
	strncpy(file->file.dir_info.path, cur_path, sizeof(file->file.dir_info.path) - 1);
...
static struct directory *create_folder(const char *path, unsigned int size)
{
	...
	strncpy(folder->folder.dir_info.path, path, sizeof(folder->folder.dir_info.path) - 1);
