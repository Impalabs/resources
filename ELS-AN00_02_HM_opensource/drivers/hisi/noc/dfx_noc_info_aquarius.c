...
/*
 * unsigned int opc: 4; bit[1-4]: Transaction operation type:
 * 0-->RD: Read operation of INCR;
 * 1-->RDW: WRAP read operation;
 * 2-->RDL: Exclusive read operation;
 * 3-->RDX: Lock read;
 * 4-->WR: INCR write operation;
 * 5-->WRW: WRAP write operation;
 * 6-->WRC: Exclusive write operation;
 * 8-->PRE: FIXED BURST;
 * 9-->URG: urgency packet, Error Probe will not detect such packets
 * Others-->Reserveed: illegal operation
 */
...
/*
 * unsigned int errcode: 3; bit[8-10]: error type
 * 0-->The visited slave returns Error Response;
 * 1-->Master has accessed the Reserve address space of the bus;
 * 2-->master sent a Burst type not supported by slave (this scenario does not appear on the current bus);
 * 3-->master visited the power-down area;
 * 4-->Access permission error;
 * 5-->When the master visits, I receive a Hide Error Response from Firewall;
 * 6-->The slave TimeOut is accessed, and Error Response is returned;
 * 7-->none
 */
...
struct noc_mid_info noc_mid_aquarius[] = {
...
struct noc_sec_info noc_sec_aquarius[] = {
...
struct noc_dump_reg noc_dump_reg_list_aquarius[] = {
...
/*
 * if noc_happend's initflow is in the dfx_modemnoc_initflow,
 * firstly save log, then system reset.
 */
const struct noc_busid_initflow dfx_filter_initflow_aquarius[] = {
...
const struct noc_bus_info noc_buses_info_aquarius[] = {
	[0] = {
		...
		.p_noc_mid_info = noc_mid_aquarius,
		.noc_mid_info_size = ARRAY_SIZE_NOC(noc_mid_aquarius),
		.p_noc_sec_info = noc_sec_aquarius,
		.noc_sec_info_size = ARRAY_SIZE_NOC(noc_sec_aquarius),
	},
	[1] = {
		.name = "vcodec_bus",
...
/* dfx_noc_get_array_size - get static array size
 * @bus_info_size : bus info array size
 * @dump_list_size: dump list array size
 */
void dfx_noc_get_array_size_aquarius(unsigned int *bus_info_size, unsigned int *dump_list_size)
{
	if ((bus_info_size == NULL) || (dump_list_size == NULL))
		return;

	*bus_info_size  = ARRAY_SIZE_NOC(noc_buses_info_aquarius);
	*dump_list_size = ARRAY_SIZE_NOC(noc_dump_reg_list_aquarius);
}

/*
 * get_reg_from_noc_device
 * @noc_dev : noc device pointer
 * @node: noc node pointer
 */
static void __iomem *get_reg_from_noc_device(struct dfx_noc_device *noc_dev,
 ...
 */
unsigned int dfx_noc_clock_enable_aquarius(struct dfx_noc_device *noc_dev,
				     struct noc_node *node)
{
	void __iomem *reg_base = NULL;
