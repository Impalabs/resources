...
struct noc_mid_info noc_mid_cows[] = {
 ...
 */
const struct noc_busid_initflow dfx_filter_initflow_cows[] = {
...
const struct noc_bus_info noc_buses_info_cows[] = {
	[0] = {
		...
		.p_noc_mid_info = noc_mid_cows,
		.noc_mid_info_size = ARRAY_SIZE_NOC(noc_mid_cows),
		.p_noc_sec_info = noc_sec_cows,
		.noc_sec_info_size = ARRAY_SIZE_NOC(noc_sec_cows),
	},
	[1] = {
		.name = "vcodec_bus",
 ...
 */
void dfx_noc_get_array_size_cows(unsigned int *bus_info_size, unsigned int *dump_list_size)
{
	if ((bus_info_size == NULL) || (dump_list_size == NULL))
		return;

	*bus_info_size  = ARRAY_SIZE_NOC(noc_buses_info_cows);
	*dump_list_size = ARRAY_SIZE_NOC(noc_dump_reg_list_cows);
}
 ...
 */
unsigned int dfx_noc_clock_enable_cows(struct dfx_noc_device *noc_dev,
