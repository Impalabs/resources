...
/* router id base address left shift bits */
#define ROUTERID_ADDR_TRANSFORM_SHIFT 4
...
/***************************************************************************
 * Function:       dfx_noc_get_array_size
 ...
 * Return:         NA
 ***************************************************************************/
void dfx_noc_get_array_size(unsigned int *bus_info_size, unsigned int *dump_list_size)
{
...
/***************************************************************************
 * Function:    dfx_noc_get_initflow_list
 ...
 ***************************************************************************/
static int dfx_noc_get_initflow_list(unsigned int busid, struct device_node *np)
{
	int ret;
	unsigned int i;
	...
	ret = of_property_read_string_array(np, "init_list", (const char **)&name[0], init_num);
	if (ret < 0) {
...
/***************************************************************************
 * Function:    dfx_noc_get_targetflow_list
 ...
 * Return:      success or fail
 ***************************************************************************/
static int dfx_noc_get_targetflow_list(unsigned int busid, struct device_node *np)
{
	int ret;
	unsigned int i;
...
/***************************************************************************
 * Function:       dfx_noc_get_routeid_addr_tbl
 ...
 * Return:         success or fail
 ****************************************************************************/
static int dfx_noc_get_routeid_addr_tbl(unsigned int busid, struct device_node *np)
{
	int i;
	int nums = 0;
	...
	for (i = 0, val = prop->value; i < nums; i++) {
		...
		if ((routeid_addr_tbl[i].init_localaddr & NOC_ROUTEID_TBL_FLAG) == NOC_ROUTEID_TBL_FLAG)
			routeid_addr_tbl[i].init_localaddr = (routeid_addr_tbl[i].init_localaddr
				- NOC_ROUTEID_TBL_FLAG) << ROUTERID_ADDR_TRANSFORM_SHIFT;
	}
...
/***************************************************************************
 * Function:       dfx_noc_get_init_target_routeid_list
 * Description:    get initflow targetflow routeid_addr from dts
 * Input:          NA
 * Output:         NA
 * Return:         success or fail
 ***************************************************************************/
static int dfx_noc_get_init_target_routeid_list(void)
{
	...
	for_each_compatible_node(np, NULL, "hisilicon,init-target-routeid-array") {
		...
		ret = dfx_noc_get_routeid_addr_tbl(busid, np);
		if (ret < 0)
			pr_err("[%s], get routeid_addr_tbl fail in dts,0x%x!\n", __func__, ret);

		ret = dfx_noc_get_initflow_list(busid, np);
		if (ret < 0)
			pr_err("[%s], get initflow_array fail in dts,0x%x!\n", __func__, ret);

		ret = dfx_noc_get_targetflow_list(busid, np);
		if (ret < 0)
			pr_err("[%s], get targetflow_array fail in dts,0x%x!\n", __func__, ret);
	}
...
static int dfx_noc_get_mid_init(unsigned int *nums, struct property **prop)
{
	struct device_node *np = NULL;
...
/***************************************************************************
 * Function:       dfx_noc_get_mid_info
 * Description:    get mid info from dts
 * Input:          platform_device
 * Output:         NA
 * Return:         success or fail
 ***************************************************************************/
static int dfx_noc_get_mid_info(void)
{
	unsigned int i;
	unsigned int nums = 0;
	...
	ret = dfx_noc_get_mid_init(&nums, &prop);
	if (ret == FAIL) {
		pr_err("[%s] dfx_noc_get_mid_init error.\n", __func__);
		return FAIL;
	}
...
/***************************************************************************
 * Function:       dfx_noc_get_dump_reg_offset
 * Description:    get dump reg offset from dts
 * Input:          platform_device
 * Output:         NA
 * Return:         success or fail
 ***************************************************************************/
static int dfx_noc_get_dump_reg_offset(void)
{
	unsigned int nums, i;
	int ret;
	struct device_node *np = NULL;
...
static int dfx_noc_get_sec_init(unsigned int *nums, struct property **prop)
{
	struct device_node *np = NULL;
...
/***************************************************************************
 * Function:       dfx_noc_get_sec_info
 * Description:    get security operation property from dts
 * Input:          platform_device
 * Output:         NA
 * Return:         success or fail
 ***************************************************************************/
static int dfx_noc_get_sec_info(void)
{
	unsigned int nums, i;
	...
	ret = dfx_noc_get_sec_init(&nums, &prop);
	if (ret == FAIL) {
		pr_err("[%s] dfx_noc_get_sec_init error.\n", __func__);
		return FAIL;
	}
...
/***************************************************************************
 * Function:       dfx_noc_get_busid_initflow
 * Description:    get busid and initflow from dts
 * Input:          platform_device
 * Output:         NA
 * Return:         success or fail
 ***************************************************************************/
static int dfx_noc_get_busid_initflow(void)
{
	unsigned int nums, i;
	struct device_node *np = NULL;
	struct property *prop = NULL;
...
static int __dfx_noc_get_bus_init(struct device_node *np, unsigned int *bus_id, char **name)
{
	int ret;
...
/***************************************************************************
 * Function:       _dfx_noc_get_bus_info
 * Description:    get static businfo dts info
 * Input:          np
 * Output:         NA
 * Return:         success or fail
 **************************************************************************/
static int __dfx_noc_get_bus_info(struct device_node *np)
{
	int ret;
	unsigned int bus_id = 0;
	char *name = NULL;

	ret = __dfx_noc_get_bus_init(np, &bus_id, &name);
	if (ret < 0) {
		pr_debug("_dfx_noc_get_bus_init fail!\n");
		return FAIL;
	}

	if (!name) {
		pr_debug("_dfx_noc_get_bus_name fail!\n");
		return FAIL;
	}
...
/***************************************************************************
 * Function:       dfx_noc_set_platform_info
 * Description:    get static platform dts info
 * Input:          np
 * Output:         NA
 * Return:         success or fail
 ***************************************************************************/
static int dfx_noc_set_platform_info(unsigned int platform_id)
{
	int ret;
	...
	g_noc_platform_info[ret].pfun_get_size = dfx_noc_get_array_size;

	return SUCCESS;
}

/***************************************************************************
 * Function:       dfx_noc_get_bus_info
 * Description:    get static bus dts info
 * Input:          pdev
 * Output:         NA
 * Return:         success or fail
 ***************************************************************************/
static int dfx_noc_get_bus_info(const struct platform_device *pdev)
{
	int ret;
	struct device_node *np = NULL;
	...
	for_each_compatible_node(np, NULL, "hisilicon,noc-bus-info") {
		ret = __dfx_noc_get_bus_info(np);
		if (ret != 0) {
			pr_err("[%s]dfx get bus info err\n", __func__);
			return FAIL;
		}
	}

	ret = dfx_noc_set_platform_info(noc_property_dt.platform_id);
	if (ret < 0) {
		pr_err("[%s]dfx_noc_set_platform_info return fail\n", __func__);
		return FAIL;
	}
...
/***************************************************************************
 * Function:       dfx_noc_get_data_from_dts
 * Description:    get static noc bus dts info
 * Input:          pdev
 * Output:         NA
 * Return:         success or fail
 ***************************************************************************/
int dfx_noc_get_data_from_dts(const struct platform_device *pdev)
{
	int ret;

	ret = dfx_noc_get_init_target_routeid_list();
	if (ret != 0) {
		pr_err("dfx_noc_init_target_routeid_list err\n");
		goto err;
	}

	ret = dfx_noc_get_mid_info();
	if (ret != 0) {
		pr_err("dfx_mid_info_list err\n");
		goto err;
	}

	ret = dfx_noc_get_dump_reg_offset();
	if (ret != 0) {
		pr_err("dfx_noc_get_dump_reg_offset err\n");
		goto err;
	}

	ret = dfx_noc_get_sec_info();
	if (ret != 0) {
		pr_err("dfx_noc_get_sec_info err\n");
		goto err;
	}

	ret = dfx_noc_get_busid_initflow();
	if (ret != 0) {
		pr_err("dfx_noc_get_busid_initflow err\n");
		goto err;
	}

	ret = dfx_noc_get_bus_info(pdev);
	if (ret != 0) {
		pr_err("dfx get bus info err\n");
		goto err;
	}
