...
void drg_cpufreq_register(struct cpufreq_policy *policy)
{
	...
	cluster = topology_physical_package_id(cpumask_first(policy->related_cpus));
	snprintf(name, sizeof(name), "/cpus/cpu-map/cluster%d", cluster);
	cn = of_find_node_by_path(name);
	if (!cn) {
		pr_err("No Cluster information found in DT\n");
		return;
	}
