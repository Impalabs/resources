 ...
 * Description: platform config file for hisp250
 * Author: sunshaodan
 * Create: 2018-08-20
 *
...
extern int hisp_secmem_size_get_from_type(unsigned int *size,
	unsigned int type);
...
#define i_2_hi(i) container_of(i, hisp250_t, intf)
 ...
 */
void hisp250_set_timestamp(unsigned int *timestamp_high, unsigned int *timestamp_low)
{
...
void hisp250_handle_msg(hisp_msg_t *msg)
{
	...
	case BATCH_REQUEST_RESPONSE:
		...
		cam_info("%s batch h:0x%x l:0x%x, rate %d",
			...
			msg->u.ack_batch_request.system_couter_rate);
		hisp250_set_timestamp(&(msg->u.ack_batch_request.timestampH),
			&(msg->u.ack_batch_request.timestampL));
		break;
	case REQUEST_RESPONSE:
		hisp250_set_timestamp(&(msg->u.ack_request.timestampH),
			&(msg->u.ack_request.timestampL));
		break;
	case MSG_EVENT_SENT:
		hisp250_set_timestamp(&(msg->u.event_sent.timestampH),
...
static void hisp250_save_rpmsg_data(const void *data, int len)
{
	...
	if (memcpy_s(skbdata, len, data, len) != 0) {
	    len = -EFAULT;
	    cam_err("Fail: %s()%d ret = %d", __FUNCTION__, __LINE__, len);
	}
...
static int hisp250_rpmsg_ept_cb(struct rpmsg_device *rpdev,
...
static int buffer_is_invalid(int share_fd,
	unsigned int req_addr, unsigned int req_size)
{
	int ret;
...
static int find_suitable_mem_pool(struct hisp_cfg_data *pcfg)
{
	...
	if (pcfg->param.type == MAP_TYPE_RAW2YUV) {
		...
		for (ipool = 0; ipool < MEM_POOL_ATTR_MAX; ipool++) {
			if (s_hisp250.mem_pool[ipool].prot ==
				pcfg->param.prot)
				break;
		}
...
static int hisp250_init_r8isp_memory_pool(void *cfg)
{
	...
	s_hisp250.mem_pool[ipool].security_isp_mode =
		pcfg->param.security_isp_mode;
	s_hisp250.mem_pool[ipool].shared_fd = pcfg->param.sharedFd;
	s_hisp250.mem_pool[ipool].is_ap_cached = pcfg->param.isApCached;
	...
	cam_info("%s: r8_iova_pool_base=0x%x",
		__FUNCTION__, s_hisp250.mem_pool[ipool].r8_iova);
	mutex_unlock(&hisi_rpmsg_service_mutex);
	return 0;
}
...
static int hisp250_deinit_r8isp_memory_pool(void *cfg)
{
	...
	memset_s(&(s_hisp250.mem_pool[ipool]), sizeof(struct hisp250_mem_pool),
		0, sizeof(struct hisp250_mem_pool));
	mutex_unlock(&hisi_rpmsg_service_mutex);
...
static int hisp250_deinit_r8isp_memory_pool_force(void)
{
	...
	for (ipool = 0; ipool < MEM_POOL_ATTR_MAX; ipool++) {
		...
		memset_s(&(s_hisp250.mem_pool[ipool]),
			sizeof(struct hisp250_mem_pool),
			0,
			sizeof(struct hisp250_mem_pool));
	}
	mutex_unlock(&hisi_rpmsg_service_mutex);
...
static int hisp250_alloc_r8isp_addr(void *cfg)
{
	...
	pcfg->param.vaddr =
		(void*)(((unsigned char*)s_hisp250.mem_pool[ipool].ap_va) +
		offset);
	pcfg->param.iova = r8_iova;
	pcfg->param.offset_in_pool = offset;
	pcfg->param.sharedFd = s_hisp250.mem_pool[ipool].shared_fd;
	pcfg->param.isApCached = s_hisp250.mem_pool[ipool].is_ap_cached;
	mutex_unlock(&hisi_rpmsg_service_mutex);
	return rc;
...
static int hisp250_set_clk_rate(int clk_level)
{
	...
	case ISP_CLK_LEVEL_TURBO:
		rc0 = hisp_set_clk_rate(ISPFUNC_CLK,
		...
		rc3 = hisp_set_clk_rate(ISPFUNC4_CLK,
	...
	if (rc0 < 0 || rc1 < 0 || rc2 < 0 || rc3 < 0) {
		cam_err("%s: set clk fail, rc:%d, %d, %d, %d",
			__FUNCTION__, rc0, rc1, rc2, rc3);
		rc = -EFAULT;
	}
	return rc;
}
...
static int hisp250_config(hisp_intf_t *i, void *cfg)
{
	...
	case HISP_CONFIG_POWER_ON:
		...
		if (!remote_processor_up) {
			...
			cam_notice("%s power on the hisp250", __FUNCTION__);
			...
			cam_warn("%s begin to power on the hisp250",
				__FUNCTION__);
			rc = hisp250_power_on(i);
		}
		mutex_unlock(&hisp_power_lock_mutex);
	...
	case HISP_CONFIG_POWER_OFF:
		mutex_lock(&hisp_power_lock_mutex);
		if (remote_processor_up) {
			cam_notice("%s power off the hisp250", __FUNCTION__);
			rc = hisp250_power_off(i);
		}
		mutex_unlock(&hisp_power_lock_mutex);
		break;
...
static int hisp250_open(hisp_intf_t *i)
{
	cam_info("%s hisp250 device open", __FUNCTION__);
...
{
	int rc = 0;
	cam_info("%s hisp250 device close", __FUNCTION__);
	mutex_lock(&hisp_power_lock_mutex);
...
#pragma GCC visibility push(default)
	hisi_serv->ept =
		rpmsg_create_ept(hisi_serv->rpdev, hisp250_rpmsg_ept_cb,
			hisi_serv, chinfo);
...
static int hisp250_rpmsg_driver_cb(struct rpmsg_device *rpdev,
	void *data, int len, void *priv, u32 src)
{
	cam_info("%s enter ... ", __FUNCTION__);
...
static int hisp250_send_rpmsg(hisp_intf_t *i, hisp_msg_t *from_user, size_t len)
{
	int rc = 0;
	hisp250_t *hi = NULL;
	struct rpmsg_hisp250_service *hisi_serv = NULL;
	...
	hisi_serv = rpmsg_local.hisi_isp_serv;
	if (hisi_serv == NULL) {
		cam_err("%s() %d failed: hisi_serv does not exist", __FUNCTION__,
			__LINE__);
		rc = -ENODEV;
		goto RET;
	}
...
static int hisp250_recv_rpmsg(hisp_intf_t *i, hisp_msg_t *user_addr, size_t len)
{
	int rc = len;
	hisp250_t *hi = NULL;
	struct rpmsg_hisp250_service *hisi_serv = NULL;
...
int hisp250_release_sec_fw_buffer(void)
{
	...
	memset_s(&(s_hisp250.mem), sizeof(struct isp_mem),
...
static struct device_attribute hisp_ddr_freq_ctrl_attr =
	...
	0660, /* 0660 for ATTR mode */
	hisp_ddr_freq_ctrl_show, hisp_ddr_freq_store);
#endif /* CONFIG_HISI_DEBUG_FS */

static int32_t hisp250_platform_probe(struct platform_device* pdev)
{
	int32_t ret = 0;
	cam_info("%s: enter", __FUNCTION__);
...
static struct platform_driver s_hisp250_driver = {
...
static int __init hisp250_init_module(void)
{
	cam_notice("%s enter", __FUNCTION__);
	return platform_driver_register(&s_hisp250_driver);
}

static void __exit hisp250_exit_module(void)
{
	cam_notice("%s enter", __FUNCTION__);
	hisp_unregister(s_hisp250.pdev);
