 ...
 * Description: Hisilicon hwcam compat 32 source file.
 * Author: yangkai
 * Create: 2016-04-01
 *
	...
	cam_buf_status_t32 __user *pdata32)
{
	...
	compat_int_t status;
	struct timeval tv;
	if (!access_ok(VERIFY_READ, pdata32, sizeof(cam_buf_status_t32)))
		return -EFAULT;
	if (pdata == NULL)
		return -EFAULT;
	...
	cam_buf_status_t32 __user *pdata32)
{
	...
	compat_int_t status;
	struct timeval tv;
	if (!access_ok(VERIFY_WRITE, pdata32, sizeof(cam_buf_status_t32)))
		return -EFAULT;
	if (pdata == NULL)
		return -EFAULT;
