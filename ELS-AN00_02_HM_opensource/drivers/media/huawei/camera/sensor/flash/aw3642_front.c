...
static int g_torch_arry[TORCH_LED_MAX] = {
	6, 11, 17, 23, 29, 35, 41, 47, 53,
	59, 65, 70, 76, 82, 88, 94, 100, 106, 111,
	117, 123, 129, 135, 141, 147, 153, 159, 165,
	170, 176, 182, 188, 193, 199, 205, 211, 217,
	223, 229, 235, 241, 247, 252, 258, 264, 270,
	276, 282, 288, 293, 299, 305, 311, 317, 323,
	329, 335, 341, 346, 353, 358, 364, 370, 376
};

/* Internal varible define */
static struct hw_aw3642_front_private_data_t g_aw3642_front_pdata;
static struct hw_flash_ctrl_t g_aw3642_front_ctrl;
static struct i2c_driver g_aw3642_front_i2c_driver;
...
static int hw_aw3642_front_clear_error_and_notify_dmd(struct hw_flash_ctrl_t
	*flash_ctrl)
{
	...
	int data)
{
	...
	if (flash_ctrl->flash_mask_enable) {
		loge_if_ret(i2c_func->i2c_write(i2c_client, REG_ENABLE,
			MODE_FLASH | TX_PIN_EN | LED_EN) < 0);
	} else {
		loge_if_ret(i2c_func->i2c_write(i2c_client, REG_ENABLE,
			MODE_FLASH | TX_PIN_DISABLE | LED_EN) < 0);
	}
	return 0;
}
...
static int hw_aw3642_front_get_dt_data(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	cam_info("%s hisi,flash_current %d, rc %d\n",
		__func__, pdata->flash_current, rc);
	if (rc < 0) {
		cam_info("%s failed %d\n", __func__, __LINE__);
	...
	struct device_attribute *attr, char *buf)
{
	...
	rc = snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE,
		...
		"mode=%d, data=%d.\n",
		g_aw3642_front_ctrl.state.mode,
		g_aw3642_front_ctrl.state.data);
	if (rc < 0) {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cdata.mode == STANDBY_MODE) {
		rc = hw_aw3642_front_off(&g_aw3642_front_ctrl);
		if (rc < 0) {
		...
		cdata.data = AW3642_FRONT_TORCH_MAX_RT_CUR;
		cam_info("%s mode = %d, max_current = %d",
			__func__, cdata.mode, cdata.data);

		rc = hw_aw3642_front_on(&g_aw3642_front_ctrl, &cdata);
		if (rc < 0) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	rc = snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE,
		...
		g_aw3642_front_ctrl.flash_mask_enable);
	if (rc < 0) {
		cam_err("%s  flash name in overflow\n", __func__);
		return -EINVAL;
	}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == '0')
		g_aw3642_front_ctrl.flash_mask_enable = false;
	else
		g_aw3642_front_ctrl.flash_mask_enable = true;
...
#ifdef CAMERA_FLASH_FACTORY_TEST
static struct device_attribute g_aw3642_front_lightness =
	__ATTR(flash_lightness_f, 0660, hw_aw3642_front_lightness_show,
	hw_aw3642_front_lightness_store);
#endif

static struct device_attribute g_aw3642_front_flash_mask =
	__ATTR(flash_mask, 0660, hw_aw3642_front_flash_mask_show,
	hw_aw3642_front_flash_mask_store);

static int hw_aw3642_front_register_attribute(struct hw_flash_ctrl_t
	*flash_ctrl, struct device *dev)
{
	...
	flash_ctrl->cdev_torch.brightness_set =
		hw_aw3642_front_torch_brightness_set;
...
#ifdef CAMERA_FLASH_FACTORY_TEST
	rc = device_create_file(dev, &g_aw3642_front_lightness);
	if (rc < 0) {
		cam_err("%s failed to creat lightness attribute", __func__);
		goto err_create_lightness_file;
	}
#endif
	rc = device_create_file(dev, &g_aw3642_front_flash_mask);
	if (rc < 0) {
...
#ifdef CAMERA_FLASH_FACTORY_TEST
	device_remove_file(dev, &g_aw3642_front_lightness);
#endif
...
static int hw_aw3642_front_match(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	loge_if_ret(i2c_func->i2c_write(i2c_client, REG_IVFM,
		UVLO_EN | IVFM_VOLTAGE | IVFM_EN) < 0);
#ifdef CAMERA_FLASH_FACTORY_TEST
	register_camerafs_attr(&g_aw3642_front_lightness);
#endif
	return 0;
}
...
static int hw_aw3642_front_remove(struct i2c_client *client)
{
	...
	g_aw3642_front_ctrl.func_tbl->flash_exit(&g_aw3642_front_ctrl);
...
static const struct i2c_device_id g_aw3642_front_id[] = {
	{ "aw3642_front", (unsigned long) &g_aw3642_front_ctrl },
	{}
};

static const struct of_device_id g_aw3642_front_dt_match[] = {
...
static struct i2c_driver g_aw3642_front_i2c_driver = {
	...
	.id_table = g_aw3642_front_id,
	.driver = {
		.name = "hw_aw3642_front",
		.of_match_table = g_aw3642_front_dt_match,
	},
};
...
{
	cam_info("%s erter\n", __func__);
	return i2c_add_driver(&g_aw3642_front_i2c_driver);
}

static void __exit hw_aw3642_front_module_exit(void)
{
	cam_info("%s enter", __func__);
	i2c_del_driver(&g_aw3642_front_i2c_driver);
}

static struct hw_flash_i2c_client g_aw3642_front_i2c_client;

static struct hw_flash_fn_t g_aw3642_front_func_tbl = {
...
static struct hw_flash_ctrl_t g_aw3642_front_ctrl = {
	.flash_i2c_client = &g_aw3642_front_i2c_client,
	.func_tbl = &g_aw3642_front_func_tbl,
	.hw_flash_mutex = &flash_mut_aw3642_front,
	.pdata = (void *)&g_aw3642_front_pdata,
