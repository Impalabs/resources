...
#define convert_to_torch(x) ((x) * 16 / 5)
...
static struct hw_mp3331_front_private_data_t g_mp3331_front_pdata;
static struct hw_flash_ctrl_t g_mp3331_front_ctrl;
static struct i2c_driver g_mp3331_front_i2c_driver;
...
static int hw_mp3331_front_clear_err_and_unlock(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	if (fault_h & (MP3331_FRONT_OVER_VOLTAGE_PROTECT |
		MP3331_FRONT_VOUT_SHORT | MP3331_FRONT_LED_SHORT)) {
		if (!dsm_client_ocuppy(client_flash)) {
			dsm_client_record(client_flash,
...
static int hw_mp3331_front_flash_mode(struct hw_flash_ctrl_t *flash_ctrl,
	int data)
{
	struct hw_flash_i2c_client *i2c_client = NULL;
	struct hw_flash_i2c_fn_t *i2c_func = NULL;
	...
	if (pdata->flash_current == FLASH_LED_LEVEL_INVALID) {
		...
		if (data > MP3331_FRONT_FLASH_MAX_CUR_LEV * MP3331_FRONT_CUR_STEP_LEV / BASE) {
			...
			current_level =
				hw_mp3331_front_find_match_current(data);
			if (current_level < 0)
				current_level =
				MP3331_FRONT_FLASH_DEFAULT_CUR_LEV;
		}
	}
...
static int hw_mp3331_front_torch_mode(struct hw_flash_ctrl_t *flash_ctrl,
	int data)
{
	struct hw_flash_i2c_client *i2c_client = NULL;
	struct hw_flash_i2c_fn_t *i2c_func = NULL;
	...
	if (pdata->torch_current == FLASH_LED_LEVEL_INVALID) {
		 ...
		 * use convert_to_torch(x) to normalize current step
		 */
		data = convert_to_torch(data);
		if (data > MP3331_FRONT_TORCH_MAX_CUR_LEV * MP3331_FRONT_CUR_STEP_LEV / BASE) {
			...
			if (current_level < 0)
				current_level =
					MP3331_FRONT_TORCH_DEFAULT_CUR_LEV;
		}
	}
	/* clear error flag,resume chip */
	rc = hw_mp3331_front_clear_err_and_unlock(flash_ctrl);
	...
	struct device_attribute *attr, char *buf)
{
	...
	ssize_t rc = snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE,
		...
		"mode = %d, data = %d",
		g_mp3331_front_ctrl.state.mode,
		g_mp3331_front_ctrl.state.data);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cdata.mode == STANDBY_MODE) {
		rc = hw_mp3331_front_off(&g_mp3331_front_ctrl);
		if (rc < 0) {
		...
		rc = hw_mp3331_front_on(&g_mp3331_front_ctrl, &cdata);
		if (rc < 0) {
...
#ifdef CAMERA_FLASH_FACTORY_TEST
static struct device_attribute g_mp3331_front_lightness =
	__ATTR(flash_lightness_f, 0660, hw_mp3331_front_lightness_show,
	hw_mp3331_front_lightness_store);
#endif

static int hw_mp3331_front_register_attribute(struct hw_flash_ctrl_t
	*flash_ctrl, struct device *dev)
{
	...
	flash_ctrl->cdev_torch.brightness_set =
		hw_mp3331_front_torch_brightness_set;
...
#ifdef CAMERA_FLASH_FACTORY_TEST
	rc = device_create_file(dev, &g_mp3331_front_lightness);
	if (rc < 0) {
...
static int hw_mp3331_front_match(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	rc = register_camerafs_attr(&g_mp3331_front_lightness);
	if (rc < 0) {
		cam_err("%s failed to creat lightness attribute", __func__);
		return -EINVAL;
	}
...
static int hw_mp3331_front_remove(struct i2c_client *client)
{
	...
	g_mp3331_front_ctrl.func_tbl->flash_exit(&g_mp3331_front_ctrl);
...
static const struct i2c_device_id g_mp3331_front_id[] = {
	{ "mp3331_front", (unsigned long) &g_mp3331_front_ctrl },
	{}
};

static const struct of_device_id g_mp3331_front_dt_match[] = {
...
static struct i2c_driver g_mp3331_front_i2c_driver = {
	...
	.id_table = g_mp3331_front_id,
	.driver = {
		.name = "hw_mp3331_front",
		.of_match_table = g_mp3331_front_dt_match,
	},
};
...
{
	cam_info("%s enter\n", __func__);
	return i2c_add_driver(&g_mp3331_front_i2c_driver);
}

static void __exit hw_mp3331_front_module_exit(void)
{
	cam_info("%s enter", __func__);
	i2c_del_driver(&g_mp3331_front_i2c_driver);
}

static struct hw_flash_i2c_client g_mp3331_front_i2c_client;

static struct hw_flash_fn_t g_mp3331_front_func_tbl = {
...
static struct hw_flash_ctrl_t g_mp3331_front_ctrl = {
	.flash_i2c_client = &g_mp3331_front_i2c_client,
	.func_tbl = &g_mp3331_front_func_tbl,
	.hw_flash_mutex = &flash_mut_mp3331_front,
	.pdata = (void *)&g_mp3331_front_pdata,
