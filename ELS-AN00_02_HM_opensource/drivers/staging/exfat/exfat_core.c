...
static s32 clr_alloc_bitmap(struct super_block *sb, u32 clu)
{
	...
	i = clu >> (p_bd->sector_size_bits + 3);
	if ((u32)i >= p_fs->map_sectors) {
		pr_err("[EXFAT] clu %u is too large", clu);
		return -EINVAL;
	}
			...
			struct chain_t *p_chain)
{
	...
	if (hint_clu == CLUSTER_32(~0)) {
		...
		/* 2 mean cluster base */
		pr_err("hint_clu = %u, ERROR", hint_clu);
		return -EPERM;
	}
...
s32 load_alloc_bitmap(struct super_block *sb)
{
	...
	while (clu.dir != CLUSTER_32(~0)) {
		...
		if (exfat_fat_read(sb, clu.dir, &clu.dir) != 0 || clu.dir == 0 ||
			cluster_count++ > p_fs->num_clusters)
			return -EIO;
	}
