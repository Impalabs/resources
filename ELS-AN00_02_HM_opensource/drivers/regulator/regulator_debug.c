...
int regulator_debugfs_create(struct device *dev)
{
	...
	if (debugfs_flag == 0) {
		...
		if (!d) {
			dev_err(dev, "failed to create regulator debugfs dir!\n");
			return -ENOMEM;
		}
	...
	enum track_regulator_type track_item, u8 mode, int max_uv, int min_uv)
{
	...
	pr_debug("######%s!\n", info.comm);
	hisiap_ringbuffer_write(
		(struct hisiap_ringbuffer_s *)g_regulator_track_info.percpu_addr[cpu],
		(u8 *)&info);
}
	...
	enum BUF_TYPE_EN buf_type)
{
	...
	for (i = 0; i < cpu_num; i++) {
		...
		ret = hisiap_ringbuffer_init((struct hisiap_ringbuffer_s *)g_regulator_track_info.percpu_addr[i],
			g_regulator_track_info.percpu_length[i], fieldcnt,
			"regulator");
		if (ret) {
