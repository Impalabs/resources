...
#define HISI_REGULATOR_MBX_ID HISI_ACPU_LPM3_MBX_2
...
static int ip_regulator_freq_autodown_clk(int regulator_id, u32 flag)
{
	...
	switch (regulator_id) {
	case VIVOBUS_ID:
		ret = freq_autodown_clk_set("vivobus", flag);
		break;
	case VCODECSUBSYS_ID:
		ret = freq_autodown_clk_set("vcodecbus", flag);
		break;
	case VDEC_ID:
		ret = freq_autodown_clk_set("vdecbus", flag);
		break;
	case VENC_ID:
		ret = freq_autodown_clk_set("vencbus", flag);
		break;
	case ISPSUBSYS_ID:
		ret = freq_autodown_clk_set("ispa7bus", flag);
		break;
	case IVP_ID:
		ret = freq_autodown_clk_set("ivpbus", flag);
		break;
#if defined(CONFIG_ATLA_IP_PLATFORM)
	case ICS_ID:
		ret = freq_autodown_clk_set("icsbus", flag);
		break;
	case ICS2_ID:
		ret = freq_autodown_clk_set("ics2bus", flag);
		break;
#endif
#if defined(CONFIG_PHOE_IP_PLATFORM)
	case VENC2_ID:
		ret = freq_autodown_clk_set("venc2bus", flag);
		break;
#endif
#if defined(CONFIG_BALT_IP_PLATFORM)
	case IVP1_ID:
		ret = freq_autodown_clk_set("ivp1bus", flag);
		break;
#endif
	default:
