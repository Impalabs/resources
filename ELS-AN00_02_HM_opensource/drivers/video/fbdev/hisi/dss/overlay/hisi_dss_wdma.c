	...
	char __iomem *wdma_base, dss_wdma_t *s_wdma)
{
	...
	if (s_wdma->hebce_used == 1)
		hisi_dss_hebce_set_reg(dpufd, wdma_base, s_wdma);
#endif
}
...
#ifdef SUPPORT_DSS_HEBCE
		ret = hisi_dss_hebce_config(dpufd, layer, aligned_rect, param.ov_block, last_block);
		dpu_check_and_return((ret != 0), -EINVAL, ERR, "hisi_dss_hebce_config failed! ret = %d\n", ret);
#endif
		return 0;
	}
