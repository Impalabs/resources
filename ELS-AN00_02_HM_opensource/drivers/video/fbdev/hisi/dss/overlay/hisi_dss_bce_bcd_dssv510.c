...
static void hisi_dss_hfbce_generate_reg_value(struct dpu_fb_data_type *dpufd, dss_wb_layer_t *layer,
...
int hisi_dss_hfbce_config(struct dpu_fb_data_type *dpufd, dss_wb_layer_t *layer, dss_rect_t aligned_rect,
	dss_rect_t *ov_block_rect, bool last_block)
{
	...
	dpu_check_and_return((!dpufd || !layer), -EINVAL, ERR, "NULL ptr\n");

	chn_idx = layer->chn_idx;
	hfbce_para.wdma_format = hisi_pixel_format_hal2dma(layer->dst.format);
	dpu_check_and_return(hfbce_para.wdma_format < 0, -EINVAL, ERR, "hisi_pixel_format_hal2dma failed!\n");

	in_rect = aligned_rect;
	hfbce_para.wdma_transform = hisi_transform_hal2dma(layer->transform, chn_idx);
	dpu_check_and_return(hfbce_para.wdma_transform < 0, -EINVAL, ERR, "hisi_transform_hal2dma failed!\n");
	...
	hisi_dss_hfbce_generate_reg_value(dpufd, layer, &hfbce_para, in_rect, last_block);
	if (g_debug_ovl_offline_composer)
...
static int hisi_dss_hebcd_calc_base_para(struct dpu_fb_data_type *dpufd, dss_layer_t *layer,
...
static int hisi_dss_hebcd_using_base_para(struct dpu_fb_data_type *dpufd, int ovl_idx,
	dss_layer_t *layer, const struct dss_hebcd_para *hebcd_para)
{
	int ret;

	if (layer->img.hebc_mmbuf_base0_y8 <= 0) {
		ret = hisi_dss_hebcd_mmbuf_config(dpufd, ovl_idx, layer,
			hebcd_para->hebcd_block_type, hebcd_para->is_yuv_semi_planar);
		if (ret < 0) {
			DPU_FB_ERR("hisi_dss_hebcd_mmbuf_config fail!\n");
			return -EINVAL;
		}
	}

	ret = hisi_dss_hebcd_config_check_mm_base0(dpufd, hebcd_para->mm_base0_y8, hebcd_para->mm_base1_c8,
...
static void hisi_dss_hebcd_generate_reg_value(struct dpu_fb_data_type *dpufd, dss_layer_t *layer,
	...
	struct hisi_ov_compose_flag *ov_compose_flag)
{
	...
	ret = hisi_dss_hebcd_calc_base_para(dpufd, layer, &hebcd_para);
	if (ret)
		return -EINVAL;

	ret = hisi_dss_hebcd_using_base_para(dpufd, ovl_idx, layer, &hebcd_para);
	if (ret)
		return -EINVAL;
	...
	hisi_dss_hebcd_generate_reg_value(dpufd, layer, &hebcd_para, ov_compose_flag->rdma_stretch_enable);
...
static void hisi_dss_hebce_generate_reg_value(struct dpu_fb_data_type *dpufd, const struct dss_hebce_para *hebce_para,
...
int hisi_dss_hebce_config(struct dpu_fb_data_type *dpufd, dss_wb_layer_t *layer,
	dss_rect_t aligned_rect, dss_rect_t *ov_block_rect, bool last_block)
{
	...
	dpu_check_and_return(hebce_para.wdma_transform < 0, -EINVAL, ERR, "hisi_transform_hal2dma failed!\n");

	hebce_para.wdma_format = hisi_pixel_format_hal2dma(layer->dst.format);
	dpu_check_and_return(hebce_para.wdma_format < 0, -EINVAL, ERR, "hisi_pixel_format_hal2dma failed!\n");
	...
	hisi_dss_hebce_generate_reg_value(dpufd, &hebce_para, layer, in_rect, last_block);
