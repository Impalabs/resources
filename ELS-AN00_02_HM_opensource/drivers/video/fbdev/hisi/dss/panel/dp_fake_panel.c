...
static struct dpu_panel_info g_fake_panel_info = {0};
/* lint -save -e* */
static struct dpu_fb_panel_data g_fake_panel_data = {
...
{
	/* init lcd info */
	struct dpu_panel_info *pinfo = g_fake_panel_data.panel_info;
...
static int dp_fake_panel_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		DPU_FB_DEBUG("dts get fake_dp fail");
		is_fake_dp = 0;
	}
	DPU_FB_INFO("[DP] is_fake_dp=%d.\n", is_fake_dp);
	if (pdev == NULL) {
		DPU_FB_ERR("NOT FOUND platform_device");
		return -1;
	}
	pdev->id = 1;
	...
	if (ret) {
		DPU_FB_ERR("[DP] platform_device_add_data failed!\n");
...
static int __init dp_fake_panel_init(void)
{
	...
	if (ret) {
		DPU_FB_ERR("[DP] register failed, error=%d!\n", ret);
		return ret;
	}
