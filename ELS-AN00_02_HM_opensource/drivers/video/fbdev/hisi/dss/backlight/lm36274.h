...
#define LM36274_EMERG(msg, ...) \
	do { \
		if (lm36274_msg_level > 0) \
			printk(KERN_EMERG "[lm36274]%s: "msg, __func__, ## __VA_ARGS__); \
	} while (0)
#define LM36274_ALERT(msg, ...) \
	do { \
		if (lm36274_msg_level > 1) \
			printk(KERN_ALERT "[lm36274]%s: "msg, __func__, ## __VA_ARGS__); \
	} while (0)
#define LM36274_CRIT(msg, ...) \
	do { \
		if (lm36274_msg_level > 2) \
			printk(KERN_CRIT "[lm36274]%s: "msg, __func__, ## __VA_ARGS__); \
	} while (0)
#define LM36274_ERR(msg, ...) \
	do { \
		if (lm36274_msg_level > 3) \
			printk(KERN_ERR "[lm36274]%s: "msg, __func__, ## __VA_ARGS__); \
	} while (0)
#define LM36274_WARNING(msg, ...) \
	do { \
		if (lm36274_msg_level > 4) \
			printk(KERN_WARNING "[lm36274]%s: "msg, __func__, ## __VA_ARGS__); \
	} while (0)
#define LM36274_NOTICE(msg, ...) \
	do { \
		if (lm36274_msg_level > 5) \
			printk(KERN_NOTICE "[lm36274]%s: "msg, __func__, ## __VA_ARGS__); \
	} while (0)
#define LM36274_INFO(msg, ...) \
	do { \
		if (lm36274_msg_level > 6) \
			printk(KERN_INFO "[lm36274]%s: "msg, __func__, ## __VA_ARGS__); \
	} while (0)
#define LM36274_DEBUG(msg, ...) \
	do { \
		if (lm36274_msg_level > 7) \
			printk(KERN_DEBUG "[lm36274]%s: "msg, __func__, ## __VA_ARGS__); \
	} while (0)
...
#define LM36274_VTC_LINE_BOOST "lm36274_vtc_line_boost"
#define LM36274_VTC_END_BOOST "lm36274_vtc_end_boost"
