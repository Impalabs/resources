		...
		struct device_attribute *attr, char *buf)
{
	...
	case TCPC_DESC_ROLE_DEF:
		size = snprintf_s(buf, MAX_PROP_LEN, MAX_PROP_LEN - 1, "%s\n"
			"1:sink, 2:source, 3:drp, 4:try source(reserved), 5:try sink\n",
...
static void tcpc_enable_timer(struct tcpc_device *tcpc, uint32_t timer_id)
{
		...
		else if (timer_id > PD_PE_TIMER_END_ID &&
				timer_id < PD_TIMER_NR)
...
static void tcpc_pd_test(struct tcpc_device *tcpc, unsigned int test_id)
{
	...
	switch (test_id) {
	case 1: /* 1: Power Role Swap */
		hisi_tcpm_power_role_swap(tcpc);
		break;

	case 2: /* 2: Data Role Swap */
	...
	case 4: /* 4: Software Reset */
		hisi_tcpm_soft_reset(tcpc);
		break;

	case 5: /* 5: Hardware Reset */
...
static inline void tcpc_set_local_rp_level(struct tcpc_device *tcpc,
		unsigned int level)
{
		...
		struct tcpc_ops *ops, void *drv_data)
{
	...
	if (!tcpc) {
		E("Allocate tcpc memeory failed\n");
		return NULL;
	}
...
int hisi_tcpc_tcpci_init(struct tcpc_device *tcpc, bool sw_reset)
{
	...
	if (tcpc->ops->init) {
		...
		E("Please implment tcpc ops init function\n");
		return -EINVAL;
	}
...
MODULE_AUTHOR("Xingliang Zhou <zhouxingliang@hisilicon.com>");
MODULE_LICENSE("GPL");
