 ...
 */
static bool dp_update_dp_connected_one(pd_port_t *pd_port,
...
bool hisi_dp_dfp_u_notify_pe_startup(pd_port_t *pd_port,
...
static bool dp_dfp_u_discover_modes(pd_port_t *pd_port)
{
	D("\n");
		...
		svdm_svid_data_t *svid_data, pd_event_t *pd_event)
{
	...
	if (pd_port->data_role != PD_ROLE_DFP)
		pr_err("the data_role %u is not %u\n",
...
{
	if  (((dp_cap >> 24) == 0) && ((dp_cap & 0x00ffffff) != 0)) /* 24 and 0x00ffffff: dp cap */
		return true;
	return false;
}
...
static void eval_dp_match_score_dp_pin_e(uint32_t *local_dp_config,
...
static void eval_dp_match_score_dp_pin_c(uint32_t *local_dp_config,
		uint32_t *remote_dp_config,
		bool remote_is_ufp_pin_assignment,
		bool local_is_dfp_pin_assignment)
{
...
static void eval_dp_match_score_dp_pin_f(uint32_t *local_dp_config,
		uint32_t *remote_dp_config,
		bool remote_is_ufp_pin_assignment,
		bool local_is_dfp_pin_assignment)
{
...
static void eval_dp_match_score_dp_pin_d(uint32_t *local_dp_config,
		uint32_t *remote_dp_config,
		bool remote_is_ufp_pin_assignment,
		bool local_is_dfp_pin_assignment)
{
...
static void eval_dp_match_score_dp_pin_a(uint32_t *local_dp_config,
		uint32_t *remote_dp_config,
		bool remote_is_ufp_pin_assignment,
		bool local_is_dfp_pin_assignment)
{
...
static void eval_dp_match_score_dp_pin_b(uint32_t *local_dp_config,
		uint32_t *remote_dp_config,
		bool remote_is_ufp_pin_assignment,
		bool local_is_dfp_pin_assignment)
{
...
static int eval_dp_match_score(uint32_t local_mode,
		...
		uint32_t *remote_dp_config)
{
	uint32_t local_pin_assignment = 0;
	uint32_t remote_pin_assignment = 0;
	...
	D("local_mode 0x%x, remote_mode 0x%x", local_mode, remote_mode);
	D("local_pin_assignment 0x%x, remote_pin_assignment 0x%x",
			local_pin_assignment, remote_pin_assignment);
	D("remote_is_ufp_pin_assignment %d, local_is_dfp_pin_assignment %d",
...
static uint8_t dp_dfp_u_select_mode(pd_port_t *pd_port,
		svdm_svid_data_t *svid_data)
{
	int i, j;
	int match_score;
	...
	svdm_mode_t *remote = NULL;
	svdm_mode_t *local = NULL;
		...
		svdm_svid_data_t *svid_data, bool ack)
{
	...
	if (pd_port->mode_obj_pos == 0) {
		...
		hisi_dp_dfp_u_set_state(pd_port,
				DP_DFP_U_ERR_DISCOVER_MODE_CAP);
		return false;
	}
...
static bool dp_dfp_u_select_pin_mode(pd_port_t *pd_port)
{
...
static bool dp_dfp_u_update_dp_connected(pd_port_t *pd_port,
		uint32_t dp_status)
{
	bool valid_connected = false;
		...
		pd_event_t *pd_event, bool ack)
{
	...
	if (oper_mode) {
		...
		valid_connected = dp_dfp_u_update_dp_connected(
				pd_port, dp_status);
		if (valid_connected)
			hisi_dp_dfp_u_request_dp_configuration(
					pd_port, pd_event);
	}
	return true;
}
...
static bool dp_ufp_u_update_dp_connected(pd_port_t *pd_port,
		uint32_t dp_status)
{
	bool valid_connected = false;
	uint32_t dp_connected, dp_local_connected;
...
#ifdef CONFIG_USB_PD_ALT_MODE_DFP_SUPPORT
void hisi_pd_dpm_dfp_send_dp_status_update(pd_port_t *pd_port,
		pd_event_t *pd_event)
{
...
void hisi_pd_dpm_dfp_send_dp_configuration(pd_port_t *pd_port,
		pd_event_t *pd_event)
{
