...
int proxy_hub_control(struct proxy_hcd *phcd, __u16 type_req, __u16 w_value,
		__u16 w_index, char *buf, __u16 w_length)
{
	struct proxy_hcd_client *client = NULL;
	...
	cmd.bRequestType = (__u8)((type_req >> BITS_PER_BYTE) & 0xff);
	cmd.bRequest = (__u8)(type_req & 0xff);
	cmd.wValue = w_value;
	cmd.wIndex = w_index;
	cmd.wLength = w_length;
