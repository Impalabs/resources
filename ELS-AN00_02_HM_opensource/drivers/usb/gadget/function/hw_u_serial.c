 ...
 */
{
	...
	while (!port->write_busy && !list_empty(pool)) {
		...
		if (status) {
			pr_debug("%s %s err %d\n", "queue", in->name, status);
			list_add(&req->list, pool);
			break;
		}
		if (port->port_num == 1) /* 1: port number */
...
static void gs_rx_push(unsigned long _port)
{
	...
	while (!list_empty(queue)) {
		...
		if (port->port_num == 1) /* 1: port number */
			pr_err("%d:len=%d,%c%c%c%c%c%c%c%c\n",
				port->port_num, req->length,
				*((u8 *)req->buf), *((u8 *)req->buf + 1),
				*((u8 *)req->buf + 2), *((u8 *)req->buf + 3),
				*((u8 *)req->buf + 4), *((u8 *)req->buf + 5),
...
static int gs_open(struct tty_struct *tty, struct file *file)
{
	...
	if (port->port_write_buf.buf_buf == NULL) {
		...
		if (status) {
			pr_debug("ttyGS%d %p,%p no buffer\n",
...
static int gs_write(struct tty_struct *tty, const unsigned char *buf, int count)
{
	...
	pr_vdebug("ttyGS%d %p writing %d bytes\n", port->port_num, tty, count);
...
static int gs_put_char(struct tty_struct *tty, unsigned char ch)
{
	...
	pr_vdebug("%d,%p char=0x%x, called from %pf\n",
...
static int gs_break_ctl(struct tty_struct *tty, int duration)
{
	...
	pr_vdebug("ttyGS%d, send break %d\n", port->port_num, duration);
...
int hw_gserial_alloc_line(unsigned char *line_num)
{
	...
	if (IS_ERR(tty_dev)) {
		...
		pr_err("failed to register tty for port %d, err %ld\n",
			port_num, PTR_ERR(tty_dev));
...
static int userial_init(void)
{
	...
	if (status) {
		pr_err("cannot register, err %d\n", status);
		goto fail;
	}

	pr_debug("registered %d ttyGS* device%s\n",
		HW_MAX_U_SERIAL_PORTS, (HW_MAX_U_SERIAL_PORTS == 1) ? "" : "s");
...
MODULE_AUTHOR("Suo An Da Jie <suoandajie@huawei.com>");
