...
static ssize_t rndis_vendor_id_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%04x\n", vendor_id);
}

static ssize_t rndis_vendor_id_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
...
static DEVICE_ATTR(vendorID, FILE_MODE, rndis_vendor_id_show,
						rndis_vendor_id_store);
...
static void rndis_set_host_ethaddr(struct f_rndis_opts *opts)
{
...
static void rndis_nop_release(struct device *dev)
{
	dev_vdbg(dev, "%s\n", __func__);
}

static void rndis_opts_set(struct f_rndis_opts *opts)
{
	if (wceis) {
...
static struct usb_function_instance *rndis_alloc_inst(void)
{
	...
	opts->dev.release = rndis_nop_release;
