...
static int gps_gpio_irq_init(int gpio)
{
	...
	if ((gpio_request(gpio, "gps_host_wake"))) {
		GPSERR("[gps]Can't request HOST_REQ GPIO %d.It may be already registered\n",
			 gpio);
		return -EIO;
	}
					...
					loff_t *off)
{
	...
	if (gps_reqrsp_data == 0) {
		tmp[0] = '0';
	} else if (gps_reqrsp_data == 1) {
		tmp[0] = '1';
	}
...
static int k3_gps_bcm_probe(struct platform_device *pdev)
{
	...
	if (ret != 0) {
		printk
			(" cannot register gps geofence wake miscdev on minor = %d ret = %d",
...
static int __init k3_gps_bcm_init(void)
{
	...
	if (!isMyConnectivityChip(CHIP_TYPE_BCM)) {
		...
		GPSINFO("gps chip type is matched with Broadcom, continue");
	}
#endif

	if (get_gps_ic_type() != GPS_IC_TYPE_4774) {
		GPSINFO("gps chip type is matched with Broadcom, but is not 4774");
		return -EINVAL;
	}
	GPSINFO("gps chip type is matched with Broadcom, and it is 4774");
