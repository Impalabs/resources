...
bool dp_factory_is_4k_60fps(uint8_t rate, uint8_t lanes, uint16_t h_active,
			    uint16_t v_active, uint8_t fps)
{
...
void dp_factory_set_link_event_no(uint32_t event_no, bool cablein, char *event,
				  int hotplug)
{
}
...
static bool dp_factory_check_test_result(struct dp_factory_priv *priv)
{
	...
	if ((priv->check_lanes_rate || priv->check_display_4k ||
		priv->check_display_60fps) &&
		priv->need_report_event && !priv->test_enable &&
		(priv->hotplug_retval == 0)) {
		if (priv->link_event_no &
		   ((1 << DP_LINK_STATE_SAFE_MODE) |
...
int dp_factory_get_test_result(char *buffer, int size)
{
	...
	if ((priv->hotplug_retval == 0)) {
		if ((priv->link_rate == 0) && (priv->link_lanes == 0))
				  ...
				  char *event, int hotplug)
{
	...
	if (event != NULL) {
		...
		memset(priv->link_event, 0,
		       DP_LINK_EVENT_BUF_MAX * DP_LINK_STATE_MAX);
	} else {
