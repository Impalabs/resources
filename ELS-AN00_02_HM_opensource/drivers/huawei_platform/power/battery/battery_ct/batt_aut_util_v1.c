...
{
	check_sn_includes_null(drv_data->sn, drv_data->sn_len);
...
void bsc_key_func(struct work_struct *work)
{
	...
	hwlog_info("checker no:%c group:%c prepare to certification\n",
...
sn_func_entry:
	...
	if (res.len > 0) {
		...
		if (send_batt_info_mesg(&info, (void *)res.data, res.len)) {
			hwlog_err("checker no:%c group:%c send attr fail %s\n",
				drv_data->id_in_grp, drv_data->id_of_grp,
				__func__);
	...
	if (bco->get_batt_sn(drv_data, &drv_data->sn_res, &drv_data->sn,
		&drv_data->sn_len)) {
		hwlog_err("battery no:%c group:%c get batt sn failed in %s\n",
			drv_data->id_in_grp, drv_data->id_of_grp, __func__);
		sn_fail++;
	...
	if (legal_sn_check_0(drv_data)) {
		hwlog_err("battery no:%c group:%c sn_checker failed in %s\n",
			drv_data->id_in_grp, drv_data->id_of_grp, __func__);
		return;
	}
}
