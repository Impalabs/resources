	...
	unsigned char *data, unsigned int len)
{
	...
	if (cmd->len > RT_MAX_DATA_LEN) {
		hwlog_err("command 0x%x len out of limit %u\n", no, cmd->len);
		return RT_COM_PARA_ERR;
	}
	...
	if (ret) {
		hwlog_err("send read cmd 0x%x/%u error %d\n", no,
			cmd->len, ret);
		return ret;
	}
	...
	hwlog_info("send read cmd 0x%x/%u success\n", no, cmd->len);
	...
	unsigned char *data, unsigned int len)
{
	...
	if (!cmd || ((len != 0) && !data))
		return RT_COM_PARA_ERR;
	if (cmd->len != len) {
		hwlog_err("command 0x%x len is error %u:%u\n", no,
			cmd->len, len);
		return RT_COM_PARA_ERR;
	}
	if (rt_check_phy_ops(&obj->phy_ops))
	...
	mutex_unlock(&obj->com_lock);
	if (ret)
		hwlog_err("send write cmd 0x%x/%u error %d\n", no,
			cmd->len, ret);
	else
		hwlog_info("send write cmd 0x%x/%u success\n", no,
			cmd->len);
