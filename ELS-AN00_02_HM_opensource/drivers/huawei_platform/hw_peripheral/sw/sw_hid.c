	...
	size_t count, unsigned char report_type)
{
	...
	sw_print_info("report num and type [ %x,%x ]\n",
...
{
	struct swhid_device *session = NULL;
...
static int swhid_probe(struct sw_device *dev)
{
	...
	hid->name[0] = 0;
	ret = snprintf_s(hid->phys, sizeof(hid->phys), sizeof(hid->phys) - 1,
		"SW%xMR", 0x1);
	if (ret < 0)
		goto err;

	ret = snprintf_s(hid->uniq, sizeof(hid->uniq), sizeof(hid->uniq) - 1,
		"SW%xMR", 0x2);
	if (ret < 0)
		goto err;
