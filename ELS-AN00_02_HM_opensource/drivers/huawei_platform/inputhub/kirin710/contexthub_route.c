...
static struct inputhub_route_table package_route_tbl[] = {
	{ ROUTE_SHB_PORT, { NULL, 0 }, { NULL, 0 }, { NULL, 0 },
		__WAIT_QUEUE_HEAD_INITIALIZER(package_route_tbl[0].read_wait) }, /* SHB 0 */
	{ ROUTE_MOTION_PORT, { NULL, 0 }, { NULL, 0}, { NULL, 0 },
		__WAIT_QUEUE_HEAD_INITIALIZER(package_route_tbl[1].read_wait) }, /* Motion 1 */
	{ ROUTE_CA_PORT, { NULL, 0 }, { NULL, 0}, { NULL, 0 },
		__WAIT_QUEUE_HEAD_INITIALIZER(package_route_tbl[2].read_wait) }, /* CA 2 */
	{ ROUTE_FHB_PORT, { NULL, 0 }, { NULL, 0}, { NULL, 0 },
		__WAIT_QUEUE_HEAD_INITIALIZER(package_route_tbl[3].read_wait) }, /* FHB 3 */
	{ ROUTE_FHB_UD_PORT, { NULL, 0 }, { NULL, 0}, { NULL, 0 },
		__WAIT_QUEUE_HEAD_INITIALIZER(package_route_tbl[4].read_wait) }, /* FHB_UD 4 */
};
...
{
	return &fingersense_data_intrans;
}
...
bool really_do_enable_disable(int *ref_cnt, bool enable, int bit)
{
	...
	if ((bit < 0) || (bit >= (sizeof(int32_t) << 3))) { /* <<3, bit should < 256 */
...
static inline void write_to_fifo(struct inputhub_buffer_pos *pwriter,
	char *buffer_begin, const char *buffer_end, char *buf, int count)
{
...
static void sensor_get_data(struct sensor_data *data)
{
	...
	if (!data || (!(data->type >= TAG_SENSOR_BEGIN && data->type < TAG_SENSOR_END)))
		return;
...
static const struct pkt_header *normalpack(const char *buf, unsigned int length)
{
	...
	if ((head->tag >= TAG_BEGIN && head->tag < TAG_END) &&
		...
		/* type must keep same with partial->partial_order, because integer promote */
		if (partial->partial_order == (uint8_t)(linker.partial_order + 1)) {
			if (linker.offset + partial_pkt_data_length <= sizeof(linker.link_buffer)) {
				...
				memcpy(linker.link_buffer + linker.offset,
					buf + sizeof(pkt_part_header_t), partial_pkt_data_length);
...
static int unpack(const void *buf, unsigned int length)
{
	...
	/* inc partial_order in header */
	((struct pkt_header *)buf)->partial_order = partial_order++;
	if (length <= MAX_SEND_LEN) {
		...
		for (send_cnt = MAX_SEND_LEN; send_cnt < (int)length;
			...
			/* inc partial_order in partial pkt */
			...
			memcpy(send_partial_buf + sizeof(pkt_part_header_t),
				...
				(unsigned long)((int)length - send_cnt)));
...
static int inputhub_mcu_write_cmd_nolock(const void *buf, unsigned int length)
{
	...
	if (!wait_for_mcu_resp_after_send(buf, unpack(buf, length), MAX_TIMEOUT_MS)) {
...
static bool is_extend_step_counter_cmd(const struct pkt_header *pkt)
{
	...
	case CMD_CMN_CONFIG_REQ:
		ret = (((pkt_cmn_motion_req_t *)pkt)->app_config[1] == TYPE_EXTEND);
		break;

	case CMD_CMN_INTERVAL_REQ:
		ret = (((pkt_cmn_interval_req_t *)pkt)->param.reserved[0] == TYPE_EXTEND);
		break;
...
int inputhub_mcu_write_cmd(const void *buf, unsigned int length)
{
	...
	if (TAG_SENSOR_BEGIN > ((struct pkt_header *)buf)->tag ||
	...
	if (!rd) {
		...
		while (retry_count--) { /* send retry 3 times */
			...
			if (inputhub_mcu_write_cmd(buf, length)) {
				...
				ret = -1;
			} else if (!wait_for_completion_timeout(&type_record.resp_complete,
				msecs_to_jiffies(iom3_timeout))) {
				hwlog_err("timeout %s, retry %d tag %d cmd %d g_iom3_state %d\n",
					__func__, retry_count, ((struct pkt_header *)buf)->tag,
	...
	unsigned int length, struct read_info *rd)
{
	...
	while (retry_count--) { /* send retry 3 times */
		if (!wait_for_mcu_resp_data_after_send(buf, unpack(buf, length),
			MAX_TIMEOUT_MS, &resp_pkt, sizeof(resp_pkt))) {
			hwlog_err("wait for tag:%s:%d\tcmd:%d resp timeout in %s\n",
				obj_tag_str[pkt->tag], pkt->tag, pkt->cmd, __func__);
			if (retry_count == 0)
...
static int report_resp_data(const struct pkt_subcmd_resp *head)
{
	...
	if (type_record.rd != NULL && type_record.pkt_info != NULL && /* check record info */
		...
		if (head->hd.length <= (MAX_PKT_LENGTH +
			sizeof(head->hd.errno) + sizeof(head->subcmd))) { /* data length ok */
			type_record.rd->errno = head->hd.errno; /* fill errno to app */
			/* fill data_length to app, data_length means data lenght below */
			type_record.rd->data_length = (head->hd.length -
				sizeof(head->hd.errno) - sizeof(head->subcmd));
			/* fill resp data to app */
			memcpy(type_record.rd->data, (char *)head + sizeof(struct pkt_subcmd_resp),
...
static bool is_aod_notifier(const struct pkt_header *head)
{
	...
	if ((head->tag == TAG_AOD) && (head->cmd == CMD_DATA_REQ) && (head->length > 0)) {
		sub_cmd = (uint32_t *)(head + 1);
		if ((*sub_cmd) == SUB_CMD_AOD_DSS_ON_REQ ||
			(*sub_cmd) == SUB_CMD_AOD_DSS_OFF_REQ) {
			...
			if ((*sub_cmd) == SUB_CMD_AOD_DSS_ON_REQ) {
				...
				__pm_wakeup_event(&wlock, jiffies_to_msecs(HZ / 2)); /* 100 / 2 */
			} else {
...
int register_mcu_event_notifier(int tag, int cmd, int (*notify) (const struct pkt_header *head))
{
	...
	list_for_each_entry_safe(pnode, n, &mcu_notifier.head, entry) {
		if ((tag == pnode->tag) && (cmd == pnode->cmd) && (notify == pnode->notify)) {
			hwlog_warn("tag = %d, cmd = %d, notify = %pK has registed in %s\n",
				tag, cmd, notify, __func__);
			/* return when already registed */
			goto out;
		}
	}
...
static void step_counter_data_process(pkt_step_counter_data_req_t *head)
{
	...
	if ((head->record_count > 0) && (head->record_count != EXT_PEDO_VERSION_2)) {
		/* skip offset of begin_time */
		motion_data[0] = MOTIONHUB_TYPE_HW_STEP_COUNTER; /* add motion type */
		/* the offset rely on sizeof enum motion_type_t of mcu, now it is 1 */
		...
		inputhub_route_write(ROUTE_MOTION_PORT, motion_data, extend_effect_len + 1);
	}

	hwlog_info("convert to standard step counter data to sensor event buffer\n");
	/* avoid report extend data to sensor HAL, convert to standard step counter data */
...
static bool is_dmd_log_data_report(const struct pkt_header *head)
{
	...
	return (head->tag == TAG_LOG) && (head->cmd == CMD_LOG_REPORT_REQ) &&
...
static int report_sensor_event_batch(int tag, int value[], int length, uint64_t timestamp)
{
	...
	if ((!(tag >= TAG_FLUSH_META && tag < TAG_SENSOR_END)) || (length > sizeof(event.value))) {
		hwlog_err("para error (tag : %d), (length : %d) in %s\n", tag, length, __func__);
		return -EINVAL;
	}
	if (tag != TAG_FLUSH_META) {
		...
		ltimestamp = 0;
		event.type = tag_to_hal_sensor_type[tag];
		event.length = 4; /* 4 length of event */
...
static int process_drop_report(const pkt_drop_data_req_t *head)
{
	...
	hwlog_info("%s: type %d, initial_speed %d, height %d, angle_pitch %d, angle_roll %d, impact %d\n",
		__func__, head->data.type, head->data.initial_speed, head->data.height,
		head->data.angle_pitch, head->data.angle_roll, head->data.material);
...
static int process_sensors_report(const struct pkt_header *head)
{
	...
	case TAG_RPC:
		...
		break;
	case TAG_DROP:
		process_drop_report((const pkt_drop_data_req_t *)head);
		break;
	default:
		break;
	}
...
static void inputhub_process_sensor_report(const struct pkt_header *head)
{
	...
	if (sensor_event->data_hd.cnt < 1) {
		...
		for (i = 0; i < sensor_event->data_hd.cnt; i++) {
			report_sensor_event_batch(head->tag, (int *)((char *)head +
...
static void inputhub_process_additional_info_report(const struct pkt_header *head)
{
	...
	if (addi_info->serial == 1) { /* create a begin event */
		...
		event.length = 12; /* length of event 12 */
	...
	info_len = head->length - 8; /* 8: length of tag */
	event.type = SENSORHUB_TYPE_ADDITIONAL_INFO;
	event.serial = addi_info->serial;
	event.sensor_type = tag_to_hal_sensor_type[head->tag];
	event.data_type = addi_info->type;
	event.length = info_len + 12; /* 12 for length of head */
...
static int inputhub_process_dmd_log_report(const struct pkt_header *head)
{
	...
	queue_delayed_work(system_power_efficient_wq, &(work->log_work),
		msecs_to_jiffies(250)); /* jiffies 250ms */
	return 0;
}
...
static int inputhub_process_fingerprint_report(const struct pkt_header *head)
{
	...
	const fingerprint_upload_pkt_t *fp_upload = (const fingerprint_upload_pkt_t *)head;

	__pm_wakeup_event(&wlock, jiffies_to_msecs(2 * HZ)); /* 2 * 100 */
	hwlog_info("fingerprint: %s: tag = %d, data:%d\n", __func__,
		fp_upload->fhd.hd.tag, fp_upload->data);
	fingerprint_data = (char *)fp_upload + sizeof(pkt_common_data_t);
	fingerprint_ipc_cgbe_abort_handle();
	if (fp_upload->fhd.hd.tag == TAG_FP)
		return inputhub_route_write(ROUTE_FHB_PORT,
			fingerprint_data, sizeof(fp_upload->data));
	else
...
static int inputhub_process_motion_report(const struct pkt_header *head)
{
	...
	hwlog_info("%s: motiontype = %d motion_result = %d, motion_status = %d\n",
		__func__, motion_data[0], /* 0:type */
		motion_data[1], motion_data[2]); /* 1:result, 2:status */
	return inputhub_route_write(ROUTE_MOTION_PORT, motion_data,
		head->length - (sizeof(pkt_common_data_t) - sizeof(struct pkt_header)));
}
...
int inputhub_route_recv_mcu_data(const char *buf, unsigned int length)
{
	...
	pkt_fingersense_data_report_req_t *zaxis_report = NULL;
	char *ca_data = NULL;
	pkt_batch_data_req_t *key_data = NULL;
	...
	if (is_fingerprint_data_report(head)) {
		...
		zaxis_report = (pkt_fingersense_data_report_req_t *)head;
