...
{
	timer_restart(pd_port->tcpc_dev, timer_id);
}

static inline void pd_enable_timer(struct pd_port *pd_port, uint32_t timer_id)
{
	timer_enable(pd_port->tcpc_dev, timer_id);
}
...
{
	timer_disable(pd_port->tcpc_dev, timer_id);
}
	...
	uint8_t msg, uint8_t size, const void *data)
{
	...
	return pd_send_extended_msg(pd_port,
...
static inline struct dpm_tcp_event *pd_get_cur_tcp_event(struct pd_port *pd_port)
{
	return dpm_get_curr_tcp_event(pd_port->devent);
}
