...
#define PD_MSG_BUF_SIZE       8
#define PD_EVENT_BUF_SIZE     16
...
extern void tcpci_wakeup_event_task(struct tcpc_device *tcpc_dev);
...
bool pd_put_cc_attach_event(struct tcpc_device *tcpc_dev, uint8_t type);
...
#else
static inline bool pd_put_cc_attach_event(struct tcpc_device *tcpc_dev, uint8_t type)
{
	return false;
}

static inline void pd_notify_usb_port_attach(struct tcpc_device *tcpc,
	enum typec_attach_type type)
{
}

static inline void pd_notify_usb_port_detach(struct tcpc_device *tcpc)
{
}

static inline void pd_notify_pe_rplvl_change(struct tcpc_device *tcpc, uint8_t cc_res)
{
}

static inline bool pd_is_pe_running(struct tcpc_device *tcpc)
{
	return false;
}

static inline void pd_handle_timer_out(struct tcpc_device *tcpc, uint32_t timer_id)
{
}
