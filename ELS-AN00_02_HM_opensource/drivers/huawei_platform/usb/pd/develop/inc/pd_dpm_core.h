...
{
	return typec_check_vbus_valid(pd_port->tcpc_dev);
}
...
static inline int pd_dpm_sink_vbus(struct pd_port *pd_port, bool en)
{
	...
	return typec_sink_vbus(pd_port->tcpc_dev, TCP_VBUS_CTRL_REQUEST,
		mv, -1);
}
...
static inline int pd_dpm_source_vbus(struct pd_port *pd_port, bool en)
{
	...
	return typec_source_vbus(pd_port->tcpc_dev, TCP_VBUS_CTRL_REQUEST,
		mv, -1);
}
