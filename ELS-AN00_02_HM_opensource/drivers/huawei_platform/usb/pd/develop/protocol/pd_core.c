...
static int pd_parse_pdata(struct pd_port *pd_port)
{
	...
	pr_info("%s\n", __func__);
	np = tcpc_get_dev_node(pd_port->tcpc_dev, "pd-data");
	if (np) {
...
static void pd_core_power_flags_init(struct pd_port *pd_port)
{
	...
	np = tcpc_get_dev_node(pd_port->tcpc_dev, "dpm_caps");
	if (!np)
		return;
...
int pd_set_data_role(struct pd_port *pd_port, uint8_t dr)
{
	...
	tcpm_dual_role_set_mode(pd_port->tcpc_dev, pd_port->data_role);
	tcpm_notify_role_swap(&pd_port->tcpc_dev->notifier,
		TCP_NOTIFY_DR_SWAP, dr);
...
int pd_set_power_role(struct pd_port *pd_port, uint8_t pr)
{
	...
	tcpm_dual_role_set_pr(pd_port->tcpc_dev, !(pd_port->power_role));
	tcpm_notify_role_swap(&pd_port->tcpc_dev->notifier,
		TCP_NOTIFY_PR_SWAP, pr);
	return ret;
}
...
static void pd_init_spec_revision(struct pd_port *pd_port)
{
	...
	if (tcpci_is_support(pd_port->tcpc_dev, TCPC_FLAGS_PD_REV30)) {
...
int pd_set_vconn(struct pd_port *pd_port, uint8_t role)
{
	...
	tcpm_dual_role_set_vconn(pd_port->tcpc_dev, en_role);
...
static inline int pd_reset_modal_operation(struct pd_port *pd_port)
{
	...
	for (i = 0; i < pd_port->svid_data_cnt; i++) {
		...
		if (svid_data->active_mode) {
			svid_data->active_mode = 0;
			tcpm_notify_exit_mode(&pd_port->tcpc_dev->notifier,
				svid_data->svid);
		}
	}
...
int pd_handle_soft_reset(struct pd_port *pd_port)
{
	...
	dpm_clear_tcp_event(pd_port->devent, TCP_DPM_RET_DROP_RECV_SRESET);
	return pd_send_sop_ctrl_msg(pd_port, PD_CTRL_ACCEPT);
}

static void pd_sync_sop_spec_revision(struct pd_port *pd_port)
{
#ifdef CONFIG_USB_PD_REV30_SYNC_SPEC_REV
...
int pd_send_msg(struct pd_port *pd_port, uint8_t sop_type,
		uint8_t msg, bool ext, uint16_t count, const uint32_t *data)
{
	...
	if (sop_type == TCPC_TX_SOP) {
		...
		else if (tcpci_is_support(tcpc_dev, TCPC_FLAGS_PD_REV30))
			pd_rev = PD_REV30;
#endif	/* CONFIG_USB_PD_REV30_SYNC_SPEC_REV */
...
#ifdef CONFIG_HW_USB_PD_REV30
int pd_send_extended_msg(struct pd_port *pd_port,
		...
		uint8_t chunk_nr, uint8_t size, const uint8_t *data)
{
	...
	*ext_hdr = PD_EXT_HEADER_CK(size, request, chunk_nr, true);
	...
	return pd_send_msg(pd_port, sop_type, msg, true, cnt, payload);
}
#endif	/* CONFIG_HW_USB_PD_REV30 */
...
int pd_send_soft_reset(struct pd_port *pd_port)
{
	...
	pd_reset_protocol_layer(pd_port, true);
	dpm_clear_tcp_event(pd_port->devent, TCP_DPM_RET_DROP_SENT_SRESET);
...
int pd_send_bist_mode2(struct pd_port *pd_port)
{
	...
	dpm_clear_tcp_event(pd_port->devent, TCP_DPM_RET_DROP_SEND_BIST);
	...
	uint8_t reply, uint8_t cnt, uint32_t *data_obj)
{
	...
	pe_vdm_state_reply_svdm_request(pd_port);
...
{
	timer_reset_pe_tmr(pd_port->tcpc_dev);
...
int pd_update_connect_state(struct pd_port *pd_port, uint8_t state)
{
	...
	PE_INFO("pd_state=%d\r\n", state);
	tcpm_notify_pd_state(&pd_port->tcpc_dev->notifier, state);
	return 0;
}
...
void pd_sync_sop_prime_spec_version(struct pd_port *pd_port, uint8_t rev)
{
#ifdef CONFIG_USB_PD_REV30_SYNC_SPEC_REV
	struct pe_data *pe_data = &pd_port->pe_data;
...
int pd_check_attached(struct pd_port *pd_port)
{
	struct tcpc_device *tcpc = NULL;
	struct pe_data *pe_data = NULL;

	if (!pd_port || !pd_port->tcpc_dev)
		return TCPM_ERROR_UNKNOWN;

	tcpc = pd_port->tcpc_dev;
	if (!tcpc->pd_inited_flag)
		return TCPM_ERROR_PE_NOT_READY;

	if (tcpc_get_attach_type(tcpc) == TYPEC_UNATTACHED)
		return TCPM_ERROR_UNATTACHED;

#ifdef CONFIG_TYPEC_CAP_CUSTOM_SRC
	if (tcpc_get_attach_type(tcpc) == TYPEC_ATTACHED_CUSTOM_SRC)
