...
static int dp_report_hpd_state(struct pd_port *pd_port, uint32_t dp_status)
{
	struct dp_data *dp_data = pd_get_dp_data(pd_port);

	/* UFP_D to DFP_D only */
	if (PD_DP_CFG_DFP_D(dp_data->local_config))
		tcpm_notify_hpd_state(&pd_port->tcpc_dev->notifier,
...
static int dp_status_update(struct pd_port *pd_port, uint32_t dp_status)
{
	DP_INFO("status0: 0x%x\n", dp_status);
	dp_report_hpd_state(pd_port, dp_status);
	return 0;
}

static int dp_report_attention(struct pd_port *pd_port, uint32_t dp_status)
{
	/* DFP_U: Not call this function during internal flow */
	DP_INFO("attention: 0x%x\n", dp_status);
	tcpm_notify_dp_attention(&pd_port->tcpc_dev->notifier, dp_status);
	return dp_report_hpd_state(pd_port, dp_status);
}

static int dp_notify_status_update_done(struct pd_port *pd_port,
...
static int dp_notify_config_done(struct pd_port *pd_port,
	uint32_t local_cfg, uint32_t remote_cfg, bool ack)
{
	 ...
	 */
	DP_INFO("config done, L:0x%x, R:0x%x, ack=%d\n",
		local_cfg, remote_cfg, ack);

	if (ack)
		tcpm_notify_dp_state(&pd_port->tcpc_dev->notifier,
			pd_port->tcpc_dev->typec_polarity,
			local_cfg);
...
void dp_dfp_u_request_dp_configuration(struct pd_port *pd_port)
{
	...
	tcpm_notify_dp_cfg_start(&pd_port->tcpc_dev->notifier);
...
bool dp_dfp_u_notify_dp_status_update(struct pd_port *pd_port, bool ack)
{
	...
	if (!ack) {
		dp_notify_status_update_done(pd_port, 0, false);
		dp_dfp_u_set_state(pd_port,
...
bool dp_dfp_u_notify_dp_configuration(struct pd_port *pd_port, bool ack)
{
	...
	dp_notify_config_done(pd_port, dp_data->local_config,
	...
	struct svdm_svid_data *svid_data)
{
	...
	case DP_DFP_U_OPERATION:
		dp_report_attention(pd_port, dp_data->remote_status);
		break;
	}
...
static inline int dp_ufp_u_request_dp_status(struct pd_port *pd_port)
{
	...
	case DP_UFP_U_OPERATION:
		ack = true;
		dp_status_update(pd_port, dp_data->remote_status);
		break;
...
static inline int dp_ufp_u_request_dp_config(struct pd_port *pd_port)
{
	...
	case DP_UFP_U_OPERATION:
		...
		if (ack) {
			...
			tcpm_notify_dp_state(&pd_port->tcpc_dev->notifier,
				pd_port->tcpc_dev->typec_polarity, dp_config);
	...
	struct pd_port *pd_port, struct svdm_svid_data *svid_data)
{
	...
	np = tcpc_get_dev_node(pd_port->tcpc_dev, "displayport");
	if (!np) {
