...
void pd_dpm_inform_cable_id(struct pd_port *pd_port, bool src_startup)
{
	...
	if (payload) {
		...
		if (pe_data->discover_id_counter >= PD_DISCOVER_ID30_COUNT)
			pd_sync_sop_prime_spec_version(pd_port, PD_REV20);
#endif	/* CONFIG_HW_USB_PD_REV30 */
	}
	...
	if (src_startup)
		...
		pe_vdm_state_dpm_informed(pd_port);
}
...
static int pps_request_thread_fn(void *param)
{
	...
	while (true) {
		...
		do {
			msleep(7*1000);
			dpm_put_tcp_event(tcpc->pd_port.devent, &tcp_event);
		...
		struct pd_port_power_caps *src_cap, uint8_t charging_policy)
{
	...
	return dpm_find_match_info(req_info,
		...
		struct pd_port_power_caps *src_cap, uint8_t charging_policy)
{
	...
	for (i = 0; i < snk_cap->nr; i++) {
		...
		find_cap = dpm_find_match_info(req_info,
...
int pd_dpm_update_tcp_req(struct pd_port *pd_port,
...
int pd_dpm_update_tcp_req_ex(struct pd_port *pd_port,
	struct dpm_tcp_pd_request_ex *pd_req)
{
...
int pd_dpm_update_tcp_req_again(struct pd_port *pd_port)
{
	bool find_cap = false;
...
void pd_dpm_snk_standby_power(struct pd_port *pd_port)
{
	...
	if (ma >= 0)
		typec_sink_vbus(pd_port->tcpc_dev, type,
			pd_port->request_v, ma);
#else
#ifdef CONFIG_USB_PD_SNK_GOTOMIN
	typec_sink_vbus(pd_port->tcpc_dev, TCP_VBUS_CTRL_REQUEST,
...
void pd_dpm_snk_hard_reset(struct pd_port *pd_port)
{
	...
	if (!ignore_hreset)
		typec_sink_vbus(pd_port->tcpc_dev, TCP_VBUS_CTRL_HRESET,
			mv, ma);
}
...
void pd_dpm_src_transition_power(struct pd_port *pd_port)
{
	...
	typec_source_vbus(pd_port->tcpc_dev, TCP_VBUS_CTRL_REQUEST,
		pd_port->request_v_new, pd_port->request_i_new);
...
{
	typec_source_vbus(pd_port->tcpc_dev, TCP_VBUS_CTRL_HRESET,
	...
	struct pd_port *pd_port, uint16_t svid, uint8_t ops)
{
	...
	tcpm_notify_enter_mode(&pd_port->tcpc_dev->notifier,
	...
	struct pd_port *pd_port, uint16_t svid, uint8_t ops)
{
	...
	if ((ops == 0) || (ops == svid_data->active_mode)) {
		...
		tcpm_notify_exit_mode(&pd_port->tcpc_dev->notifier, svid);
		return true;
	}
...
void pd_dpm_ufp_recv_uvdm(struct pd_port *pd_port)
{
	...
	if (svid_data) {
		...
		tcpm_notify_uvdm(&pd_port->tcpc_dev->notifier, true,
			pd_port->uvdm_cnt, pd_port->uvdm_svid,
			pd_port->uvdm_data);
	} else {
...
void pd_dpm_dfp_inform_uvdm(struct pd_port *pd_port, bool ack)
{
	...
	tcpm_notify_uvdm(&pd_port->tcpc_dev->notifier, ack,
		pd_port->uvdm_cnt, pd_port->uvdm_svid,
		pd_port->uvdm_data);
	dpm_reply_tcp_event_2nd(pd_port->devent,
...
{
	/* iSnkSwapStdby : 2.5mA */
	typec_sink_vbus(pd_port->tcpc_dev, TCP_VBUS_CTRL_PR_SWAP,
		TCPC_VBUS_SINK_0V, 0);
}

void pd_dpm_prs_enable_power_source(struct pd_port *pd_port, bool en)
{
	int vbus = en ? TCPC_VBUS_SOURCE_5V : TCPC_VBUS_SOURCE_0V;

	if (!pd_port)
		return;

	typec_source_vbus(pd_port->tcpc_dev, TCP_VBUS_CTRL_PR_SWAP, vbus, -1);
...
{
	struct pd_battery_info *bat_info = NULL;
	const struct pd_battery_capabilities *bcdb = NULL;
	...
	if (bat_info != NULL) {
		tcpm_notify_request_bat_info(&pd_port->tcpc_dev->notifier,
			gbcdb->bat_cap_ref);
...
{
	struct pd_battery_capabilities *bcdb = NULL;
...
void pd_dpm_inform_alert_msg(struct pd_port *pd_port)
{
	...
	tcpm_notify_alert(&pd_port->tcpc_dev->notifier, data[0]);
}
#endif	/* CONFIG_USB_PD_REV30_ALERT_REMOTE */

#ifdef CONFIG_USB_PD_REV30_ALERT_LOCAL
int pd_dpm_send_alert_msg(struct pd_port *pd_port)
{
...
void pd_dpm_inform_status(struct pd_port *pd_port)
{
	...
	if (dpm_check_ext_msg_event(pd_port, PD_EXT_STATUS)) {
		...
		tcpm_notify_status(&pd_port->tcpc_dev->notifier, sdb);
	}
}
#endif /* CONFIG_USB_PD_REV30_STATUS_REMOTE */

#ifdef CONFIG_USB_PD_REV30_STATUS_LOCAL
int pd_dpm_send_status_msg(struct pd_port *pd_port)
{
	struct pd_status sdb;
	struct pe_data *pe_data = &pd_port->pe_data;
...
{
	struct pd_pps_status_raw *ppssdb = NULL;
...
void pd_dpm_dynamic_disable_vconn(struct pd_port *pd_port)
{
	...
	if (!dpm_tcp_event_empty(tcpc->pd_port.devent))
		return;
...
int pd_dpm_core_init(struct pd_port *pd_port)
{
	...
	ret = dpm_event_init(pd_port);
	if (ret) {
		devm_kfree(&pd_port->tcpc_dev->dev, pd_port->svid_data);
		pd_port->svid_data = NULL;
		return ret;
	}
