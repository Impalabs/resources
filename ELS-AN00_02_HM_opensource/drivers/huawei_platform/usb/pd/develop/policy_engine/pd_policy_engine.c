...
#endif	/* ndef CONFIG_HW_USB_PD_REV30 */
...
static inline uint8_t pd_try_get_deferred_tcp_event(struct pd_port *pd_port)
{
	...
	id = dpm_pop_tcp_event(pd_port->devent);
	if (id == TCP_DPM_EVT_UNKONW)
...
bool pe_state_ready(struct pd_port *port)
{
	...
	ready_state = pe_get_curr_ready_state(port);
	return port->pe_state_curr == ready_state;
}
