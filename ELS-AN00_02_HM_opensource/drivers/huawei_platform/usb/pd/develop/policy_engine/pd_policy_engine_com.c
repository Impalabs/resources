...
{
	return pd_get_cur_tcp_event(pd_port)->tcp_data.data_object;
}
...
void pe_power_ready_entry(struct pd_port *pd_port)
{
	...
	dpm_reply_tcp_event_2nd(pd_port->devent, TCP_DPM_RET_SUCCESS);
}
...
void pe_get_battery_cap_entry(struct pd_port *pd_port)
{
	...
	pd_send_sop_ext_msg(pd_port, PD_EXT_GET_BAT_CAP,
		sizeof(*gbcdb), gbcdb);
}
...
void pe_get_battery_status_entry(struct pd_port *pd_port)
{
	...
	pe_state_wait_msg(pd_port);
	pd_send_sop_ext_msg(pd_port, PD_EXT_GET_BAT_STATUS,
		sizeof(*gbsdb), gbsdb);
}
...
void pe_get_manufacturer_info_entry(struct pd_port *pd_port)
{
	...
	pe_state_wait_msg(pd_port);
	pd_send_sop_ext_msg(pd_port, PD_EXT_GET_MFR_INFO,
		sizeof(*gmidb), gmidb);
}
