	...
	struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	if (pd_event->event_type == PD_EVT_DATA_MSG ||
		...
		pd_dpm_inform_alert_msg(pd_port);
	...
	struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	if (!tcpci_is_support(pd_port->tcpc_dev, TCPC_FLAGS_RETRY_CRC_DISCARD)) {
		pe_transit_soft_reset_state(pd_port);
		return true;
	}
	...
	dpm_clear_tcp_event(pd_port->devent, TCP_DPM_RET_DROP_UNEXPECTED);
	return true;
}
...
bool pd_process_tx_failed_discard(struct pd_port *pd_port, uint8_t msg)
{
	...
	if ((msg == PD_HW_TX_DISCARD) &&
		tcpci_is_support(pd_port->tcpc_dev,
		TCPC_FLAGS_RETRY_CRC_DISCARD)) {

		dpm_clear_tcp_event(pd_port->devent, TCP_DPM_RET_DROP_DISCARD);
			...
			struct pd_event *pd_event, struct pd_msg *pd_msg)
{
	...
	if (pd_msg->frame_type == TCPC_TX_SOP_PRIME) {
		pd_sync_sop_prime_spec_version(
