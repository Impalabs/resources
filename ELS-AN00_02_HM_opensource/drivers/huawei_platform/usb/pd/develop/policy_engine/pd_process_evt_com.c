	...
	struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	case PD_CTRL_REJECT:
		dpm_reply_tcp_event_2nd(pd_port->devent, TCP_DPM_RET_REJECT);
		...
		break;
	case PD_CTRL_WAIT:
		dpm_reply_tcp_event_2nd(pd_port->devent, TCP_DPM_RET_WAIT);
	...
	struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	case PD_TIMER_SENDER_RESPONSE:
		...
		pd_cancel_dpm_reaction(pd_port);
		pd_notify_pe_cancel_pr_swap(pd_port);
		dpm_reply_tcp_event_2nd(pd_port->devent, TCP_DPM_RET_TIMEOUT);
		if (pd_port->pe_data.pe_state_flags &
			PE_STATE_FLAG_BACK_READY_IF_SR_TIMER_TOUT) {
...
#if defined(CONFIG_HW_USB_PD_REV30) && defined(CONFIG_USB_PD_REV30_COLLISION_AVOID)
	case PD_TIMER_DEFERRED_EVT:
		dpm_clear_tcp_event(pd_port->devent, TCP_DPM_RET_DROP_PE_BUSY);
		break;
#endif
