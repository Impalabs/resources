...
static inline int pd_handle_tcp_event_req(struct pd_port *pd_port)
{
	int ret = 0;
	struct dpm_tcp_event *tcp_event = pd_get_cur_tcp_event(pd_port);
	...
	case TCP_DPM_EVT_REQUEST:
		ret = pd_dpm_update_tcp_req(
			pd_port, &tcp_event->tcp_data.pd_req);
		break;
	case TCP_DPM_EVT_REQUEST_EX:
		ret = pd_dpm_update_tcp_req_ex(
			pd_port, &tcp_event->tcp_data.pd_req_ex);
		break;
	case TCP_DPM_EVT_REQUEST_AGAIN:
		ret = pd_dpm_update_tcp_req_again(pd_port);
		break;
	}
...
static inline int pd_handle_tcp_event_alert(struct pd_port *pd_port)
{
	...
	if (pd_get_curr_pd_event(pd_port)->msg_sec == PD_TCP_FROM_TCPM)
		pd_port->pe_data.local_alert |= tcp_event->tcp_data.index;
...
#ifdef CONFIG_USB_PD_PE_SINK
	case TCP_DPM_EVT_REQUEST:
		ret = pd_handle_tcp_event_req(pd_port);
		break;
	case TCP_DPM_EVT_REQUEST_EX:
		ret = pd_handle_tcp_event_req(pd_port);
		break;
...
bool pd_process_event_tcp(struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	dpm_reply_tcp_event_1st(pd_port->devent, ret);
