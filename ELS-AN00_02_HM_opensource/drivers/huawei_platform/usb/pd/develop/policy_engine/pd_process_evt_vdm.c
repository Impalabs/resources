	...
	uint8_t vdm_cmdt, const struct vdm_state_transition *state_transition)
{
	...
	dpm_reply_tcp_event_2nd(pd_port->devent, ret_code);
...
{
	int i;
	bool check_tx = transit_type >= VDM_STATE_TRANSIT_CHECK_TX;
	uint8_t vdm_cmdt;
	const struct vdm_state_transition *state_transition = NULL;
		...
		struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	dpm_reply_tcp_event_1st(pd_port->devent, tcp_ret);
	return ret;
}
...
static inline void pd_parse_tcp_dpm_evt_svdm(struct pd_port *pd_port)
{
	struct dpm_tcp_svdm_data *svdm_data =
		&pd_get_cur_tcp_event(pd_port)->tcp_data.svdm_data;
...
{
	struct dpm_tcp_dp_data *dp_data_tcp =
		&pd_get_cur_tcp_event(pd_port)->tcp_data.dp_data;
...
static inline void pd_parse_tcp_dpm_evt_uvdm(struct pd_port *pd_port)
{
	struct dpm_tcp_cust_vdm_data *vdm_data =
		&pd_get_cur_tcp_event(pd_port)->tcp_data.vdm_data;
		...
		struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	if (!pe_state_ready(pd_port)) {
		dpm_reply_tcp_event_1st(pd_port->devent,
	...
	dpm_reply_tcp_event_1st(pd_port->devent, TCP_DPM_RET_SENT);
	return true;
}
