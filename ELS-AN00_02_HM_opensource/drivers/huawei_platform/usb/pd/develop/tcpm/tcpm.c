	...
	struct notifier_block *nb, uint8_t flags)
{
	...
	return tcpm_register_notifier(&tcp_dev->notifier, nb, flags);
}
	...
	struct notifier_block *nb, uint8_t flags)
{
	...
	tcpm_unregister_dev_notifier(&tcp_dev->notifier, nb, flags);
	return 0;
}
...
int tcpm_shutdown(struct tcpc_device *tcpc_dev)
{
	...
	typec_disable_vbus_control(tcpc_dev);
...
int tcpm_inquire_typec_remote_rp_curr(struct tcpc_device *tcpc_dev)
{
	...
	if (tcpc_get_attach_type(tcpc_dev) == TYPEC_UNATTACHED)
		return 0;
...
{
	return typec_inquire_vbus_level(tcpc_dev, from_ic);
}
...
uint8_t tcpm_inquire_typec_attach_state(
	struct tcpc_device *tcpc_dev)
{
	return tcpc_get_attach_type(tcpc_dev);
}
	...
	struct tcpc_device *tcpc_dev, int curr)
{
	...
	tcpc_set_sink_curr(tcpc_dev, curr);
	if (tcpc_dev->typec_remote_rp_level != TYPEC_CC_VOLT_SNK_DFT)
		force_sink_vbus = false;

	if (force_sink_vbus)
		typec_sink_vbus(tcpc_dev, TCP_VBUS_CTRL_TYPEC,
			TCPC_VBUS_SINK_5V, -1);
	...
	struct tcpc_device *tcpc_dev, uint8_t level)
{
	...
	return tcpc_set_rp_level(tcpc_dev, res);
}
...
int tcpm_typec_role_swap(struct tcpc_device *tcpc_dev)
{
	...
	return tcpc_swap_typec_role(tcpc_dev);
}
...
int tcpm_typec_notify_direct_charge(void *client, bool dc)
{
	...
	TCPC_INFO("%s,%d", __func__ , (int)dc);
	tcpm_huawei_set_scp_charge_en(tcpc_dev, dc);
	return tcpm_set_direct_charge_en(tcpc_dev, dc);
}
...
{
	if (disable)
		tcpc_typec_disable(tcpc_dev);
	else
		tcpc_typec_enable(tcpc_dev);

	return 0;
}

extern struct tcpc_device *notify_tcp_dev_ready(const char *name)
{
 	return tcpc_dev_get_by_name(name);
}
...
int tcpm_get_remote_pwr_cap(struct tcpc_device *tcpc_dev,
...
static inline int tcpm_send_sync_tcp_event(struct tcpc_device *tcpc,
	...
	uint8_t *data, uint8_t size)
{
	...
	return dpm_send_sync_tcp_event(tcpc->pd_port.devent,
		event, tout_ms, data, size);
}

static inline int tcpm_send_async_tcp_event(struct tcpc_device *tcpc,
	...
	const struct dpm_tcp_event_cb_data *cb_data)
{
	...
	return dpm_send_async_tcp_event(tcpc->pd_port.devent,
		event, cb_data);
}
...
{
	if (!cb_data)
		return tcpm_send_sync_tcp_event(tcpc, event, tout_ms,
			NULL, 0);

	return tcpm_send_async_tcp_event(tcpc, event, cb_data);
}

int tcpm_dpm_pd_power_swap(struct tcpc_device *tcpc,
	uint8_t role, const struct dpm_tcp_event_cb_data *cb_data)
{
...
int tcpm_dpm_pd_request(struct tcpc_device *tcpc,
	int mv, int ma, const struct dpm_tcp_event_cb_data *cb_data)
{
	struct dpm_tcp_event tcp_event = {
		.event_id = TCP_DPM_EVT_REQUEST,

		.tcp_data.pd_req.mv = mv,
		.tcp_data.pd_req.ma = ma,
	};
	...
	const struct dpm_tcp_event_cb_data *cb_data)
{
	struct dpm_tcp_event tcp_event = {
		...
		.tcp_data.pd_req_ex.pos = pos,
	};

	if (oper > max)
		return TCPM_ERROR_PARAMETER;

	tcp_event.tcp_data.pd_req_ex.max = max;
	tcp_event.tcp_data.pd_req_ex.oper = oper;
...
{
	if (!cb_data)
		return tcpm_send_sync_tcp_event(tcpc, event,
			tout_ms, data, size);

	return tcpm_send_async_tcp_event(tcpc, event, cb_data);
}

int tcpm_dpm_pd_get_source_ext_cap(struct tcpc_device *tcpc,
	const struct dpm_tcp_event_cb_data *cb_data,
...
int tcpm_dpm_get_pd_status(struct tcpc_device *tcpc,
	const struct dpm_tcp_event_cb_data *cb_data, struct pd_status *status)
{
	struct dpm_tcp_event tcp_event = {
...
int tcpm_dpm_get_pps_status(struct tcpc_device *tcpc,
	const struct dpm_tcp_event_cb_data *cb_data,
...
int tcpm_dpm_pd_alert(struct tcpc_device *tcpc,
	uint32_t ado, const struct dpm_tcp_event_cb_data *cb_data)
{
	struct dpm_tcp_event tcp_event = {
...
int tcpm_dpm_pd_error_recovery(struct tcpc_device *tcpc)
{
	...
	if (dpm_put_event(tcpc->pd_port.devent, &tcp_event) != TCPM_SUCCESS)
...
int tcpm_dpm_vdm_discover_mode(struct tcpc_device *tcpc,
	uint16_t svid, const struct dpm_tcp_event_cb_data *cb_data)
{
	struct dpm_tcp_event tcp_event = {
		.event_id = TCP_DPM_EVT_DISCOVER_MODES,
		.tcp_data.svdm_data.svid = svid,
	};
	...
	const struct dpm_tcp_event_cb_data *cb_data)
{
	struct dpm_tcp_event tcp_event = {
		...
		.tcp_data.dp_data.val = dp_status,
		.tcp_data.dp_data.mask = mask,
	};
	...
	const struct dpm_tcp_event_cb_data *cb_data)
{
	struct dpm_tcp_event tcp_event = {
		...
		.tcp_data.dp_data.val = dp_config,
		.tcp_data.dp_data.mask = mask,
	};
	...
	const struct dpm_tcp_event_cb_data *cb_data)
{
	...
	memcpy(&tcp_event.tcp_data.vdm_data,
		vdm_data, sizeof(struct dpm_tcp_cust_vdm_data));
	...
	struct tcpc_device *tcpc_dev)
{
	...
	timer_disable(tcpc_dev, PD_TIMER_VBUS_STABLE);
#endif
...
uint8_t tcpm_inquire_pd_charge_policy(struct tcpc_device *tcpc)
{
	struct pd_port *pd_port = &tcpc->pd_port;
...
uint8_t tcpm_inquire_pd_charge_policy_default(struct tcpc_device *tcpc)
{
	struct pd_port *pd_port = &tcpc->pd_port;
...
int tcpm_reset_pd_charging_policy(struct tcpc_device *tcpc,
	const struct dpm_tcp_event_cb_data *cb_data)
{
	return tcpm_set_pd_charge_policy(
		tcpc, tcpc->pd_port.dpm_charging_policy_default, cb_data);
}

int tcpm_set_pd_charge_policy_default(
...
int tcpm_set_pd_charge_policy(struct tcpc_device *tcpc,
	uint8_t policy, const struct dpm_tcp_event_cb_data *cb_data)
{
	struct dpm_tcp_event tcp_event = {
...
void tcpm_huawei_set_scp_charge_en(struct tcpc_device *tcpc, bool en)
{
	struct pd_port *pd_port = &tcpc->pd_port;

	mutex_lock(&pd_port->pd_lock);
	tcpc->is_huawei_scp_chg = en;
	mutex_unlock(&pd_port->pd_lock);
}

bool tcpm_inquire_during_huawei_scp_charge(struct tcpc_device *tcpc)
{
	return tcpc->is_huawei_scp_chg;
}
...
static int tcpm_put_tcp_dummy_event(struct tcpc_device *tcpc)
{
	...
	return dpm_put_event(tcpc->pd_port.devent, &tcp_event);
}
...
int tcpm_set_apdo_charge_policy(struct tcpc_device *tcpc,
...
int tcpm_inquire_source_apdo(struct tcpc_device *tcpc,
	...
	enum pd_present_temperature_flag ptf_new, uint8_t temperature)
{
	...
	if (ptf_new < PD_PTF_NO_SUPPORT ||
	    ptf_new > PD_PTF_OVER_TEMP)
		return TCPM_ERROR_PARAMETER;
