...
static void ufs_polling_pmc_idle(struct hufs_host *host)
{
	...
	while (retry--) {
		value = readl(
			SOC_PMCTRL_QIC_POWER_IDLEACK_0_ADDR(host->pmctrl));
	...
	retry = POLLING_RETRY;
	while (retry--) {
		value = readl(SOC_PMCTRL_QIC_POWER_IDLE_0_ADDR(host->pmctrl));
...
int hufs_resume(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	...
	/* Step 2. enable the clk of UFS side of X2X async bridge */
	ufs_pericrg_set_bits(host, BIT(GT_CLK_HSDTBUS),
			SOC_CRG_GENERAL_SEC_PERCLKEN1);
	/* Step 3. release the reset of UFS side of X2X async bridge */
	ufs_pericrg_set_bits(host, BIT(IP_RST_HSDTBUS),
			SOC_CRG_GENERAL_SEC_RSTDIS1);
	/* enable ip_rst_hsdt reset HSTD subsystem */
	ufs_pericrg_set_bits(host, BIT(IP_RST_HSDT),
			SOC_CRG_GENERAL_SEC_RSTDIS);
...
void hufs_device_hw_reset(struct ufs_hba *hba)
{
	...
	if (!ufshcd_is_hufs_hc(hba))
		return hufs_device_hw_reset_hybrid(hba);
	...
	struct ufs_hba *hba, struct ufs_pa_layer_attr *dev_req_params)
{
	...
	if (ufshcd_is_hufs_hc(hba))
		return ufs_pwr_change_pre_change(hba,
							    dev_req_params);
