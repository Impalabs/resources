...
#define MPHY_TX_DIRECT_CONF6 0x8006
#define MPHY_RX_RST_CFGA 0x800a
#define MPHY_UPDATA_REG 0xd085
#define MPHY_TX_OVERIDE_DISABLE  0x43
#define MPHY_TX_OVERIDE_MASK 0xffffff7f
#define MPHY_RX_OVERIDE_DISABLE  0x30
...
static void set_rhold(struct hufs_host *host)
{
...
void ufs_hisi_mphy_clk_set(struct hufs_host *host)
{
	u32 reg;
	if (!host)
		return;
...
int hufs_link_startup_pre_change(struct ufs_hba *hba)
{
	...
	if (host->caps & USE_ONE_LANE) {
		/* Tx Rx set 1 lane */
		value = 0;
		ufshcd_dme_get(hba, UIC_ARG_MIB_SEL(MPHY_TX_DIRECT_CONF6, 0x1), &value);
		value = value | MPHY_TX_OVERIDE_DISABLE;
		value = value & MPHY_TX_OVERIDE_MASK;
		ufshcd_dme_set(hba, UIC_ARG_MIB_SEL(MPHY_TX_DIRECT_CONF6, 0x1), value);
		value = 0;
		ufshcd_dme_get(hba, UIC_ARG_MIB_SEL(MPHY_RX_RST_CFGA, 0x5), &value);
		value = value | MPHY_RX_OVERIDE_DISABLE;
		ufshcd_dme_set(hba, UIC_ARG_MIB_SEL(MPHY_RX_RST_CFGA, 0x5), value);
		ufshcd_dme_set(hba, UIC_ARG_MIB_SEL(MPHY_UPDATA_REG, 0x0), 0x1);
	}
	if (!(host->caps & USE_HUFS_MPHY_TC)) {
