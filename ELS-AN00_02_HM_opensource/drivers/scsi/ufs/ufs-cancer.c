...
static void ufs_kirin_subsys_clk_config(struct hufs_host *host)
{
...
static void ufs_kirin_bypass_ufs_clk_gate(struct hufs_host *host)
{
...
static void ufs_kirin_psw_config(struct hufs_host *host)
{
	/* open psw clk */
...
static void ufs_kirin_config_rx_status(struct ufs_hba *hba)
{
	struct hufs_host *host = (struct hufs_host *)hba->priv;
	uint32_t value1 = 0;
	uint32_t value2 = 0;
...
static void ufs_kirin_unipro_vs_mphy_disable(struct ufs_hba *hba)
{
	uint32_t value1 = 0;
...
static void ufs_kirin_set_rx_hs_g1_prepare_length_capability(
	struct ufs_hba *hba)
{
...
static void ufs_kirin_pa_tx_hs_adapt_type_config(struct ufs_hba *hba)
{
...
static void ufs_kirin_set_synclength(struct ufs_hba *hba)
{
	/* Gear3 Synclength */
...
static int ufs_kirin_dme_setup_hci_asic_mphy(struct ufs_hba *hba)
{
	int err;
	...
	ufs_kirin_config_rx_status(hba);
	...
	ufs_kirin_set_rx_hs_g1_prepare_length_capability(hba);

	ufs_kirin_pa_tx_hs_adapt_type_config(hba);
	...
	/* Unipro VS_mphy_disable */
	ufs_kirin_unipro_vs_mphy_disable(hba);
...
int hufs_link_startup_pre_change(struct ufs_hba *hba)
{
	...
	if (!(host->caps & USE_HUFS_MPHY_TC)) {
		err = ufs_kirin_dme_setup_hci_asic_mphy(hba);
		if (err)
			return err;
	}
	...
	/* for hufs MPHY */
	hufs_mphy_updata_vswing_fsm_ocs5(hba, host);
...
static void ufs_kirin_hufs_mphy_link_post_config(struct ufs_hba *hba)
{
	struct hufs_host *host = (struct hufs_host *)hba->priv;
...
int hufs_link_startup_post_change(struct ufs_hba *hba)
{
	...
	/* for hufs MPHY */
	ufs_kirin_hufs_mphy_link_post_config(hba);
