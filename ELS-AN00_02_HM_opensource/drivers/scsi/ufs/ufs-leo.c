...
static void ufs_kirin_set_memory_sd_signal_0(struct hufs_host *host)
{
	u32 reg;
...
static void ufs_kirin_memory_repair(struct hufs_host *host)
{
...
static void ufs_kirin_mphy_clk_set(struct hufs_host *host)
{
	u32 reg;
...
static void ufs_cfg_ip_reset(struct hufs_host *host)
{
	u32 reg;
...
static void mphy_amplitude_glitch_workaround(struct ufs_hba *hba)
{
	...
	/* tx_ana_ctrl_leg_pull_en, tx_ana_ctrl_post */
	const uint16_t(*table)[TABLE_DB_CONFIG_SIZE] = NULL; /* define 2 */
...
void set_device_clk(struct ufs_hba *hba)
{
	...
	pmic_write_reg(PMIC_NP_CLK_UFS_FRE_CTRL_ADDR(0), ufs_refclk_val);
	/* open the device clk */
	pmic_write_reg(PMIC_CLK_UFS_EN_ADDR(0), 1);
