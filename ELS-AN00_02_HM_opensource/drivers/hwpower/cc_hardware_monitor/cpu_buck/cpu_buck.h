...
#define DELAY_TIME_FOR_WORK        5000
...
#ifdef CONFIG_HUAWEI_CPU_BUCK
void cpu_buck_str_to_reg(const char *str, char *reg, int size);
void cpu_buck_register(struct cpu_buck_sample *cbs);
#else
static inline void cpu_buck_str_to_reg(const char *str, char *reg, int size)
{
}

static inline void cpu_buck_register(struct cpu_buck_sample *cbs)
{
}
#endif /* CONFIG_HUAWEI_CPU_BUCK */
