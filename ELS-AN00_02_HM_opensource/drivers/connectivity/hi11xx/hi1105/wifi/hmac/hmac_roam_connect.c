...
static void hmac_roam_process_addba_action(uint8_t *payload, hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user,
    uint32_t *ret, uint32_t frame_body_len)
{
    oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ROAM,
...
OAL_STATIC uint32_t hmac_roam_process_action(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    if (payload[MAC_ACTION_OFFSET_CATEGORY] == MAC_ACTION_CATEGORY_BA) {
        hmac_roam_process_addba_action(payload, hmac_vap, hmac_user, &ret, frame_body_len);
    }
