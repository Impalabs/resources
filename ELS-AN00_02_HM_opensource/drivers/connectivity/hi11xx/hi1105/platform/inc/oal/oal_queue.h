...
OAL_STATIC OAL_INLINE uint32_t oal_queue_init(oal_queue_stru *pst_queue, uint8_t uc_max_events)
{
    ...
    if (uc_max_events == 0) {
        ...
        pul_buf = (uintptr_t *)oal_mem_alloc_m(OAL_MEM_POOL_ID_LOCAL,
                                               (uint16_t)(uc_max_events * sizeof(uintptr_t)), OAL_TRUE);
 ...
 * 输入参数  : pst_queue: 队列指针
 */
OAL_STATIC OAL_INLINE void oal_queue_destroy(oal_queue_stru *pst_queue)
{
    if (pst_queue == NULL) {
        return;
    }
