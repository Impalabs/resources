...
OAL_STATIC OAL_INLINE void oal_l1cache_flush(void)
{
 ...
 */
OAL_STATIC OAL_INLINE void oal_l2cache_dcache_lock(oal_bool_enum_uint8 en_flag, uint32_t ul_way)
{
    uint32_t ul_reg;
 ...
 *             ul_way: 要锁住或者释放的对应的cache way
 */
OAL_STATIC OAL_INLINE void oal_l2cache_icache_lock(oal_bool_enum_uint8 en_flag, uint32_t ul_way)
{
    uint32_t ul_reg;
#if ((_PRE_TARGET_PRODUCT_TYPE_5610EVB == _PRE_CONFIG_TARGET_PRODUCT) || \
     (_PRE_TARGET_PRODUCT_TYPE_VSPM310DMB == _PRE_CONFIG_TARGET_PRODUCT) || \
 ...
 */
OAL_STATIC OAL_INLINE void oal_cache_flush_all(void)
{
    flush_cache_all();
}
 ...
 */
OAL_STATIC OAL_INLINE void oal_cache_rxtx_lock(void)
{
#if ((_PRE_TARGET_PRODUCT_TYPE_5610EVB == _PRE_CONFIG_TARGET_PRODUCT) || \
     (_PRE_TARGET_PRODUCT_TYPE_VSPM310DMB == _PRE_CONFIG_TARGET_PRODUCT) || \
     (_PRE_TARGET_PRODUCT_TYPE_WS835DMB == _PRE_CONFIG_TARGET_PRODUCT))
    l2cache_base = (void __iomem *)ioremap(REG_BASE_L2CACHE, sizeof(void *));
#else
    l2cache_base = (void __iomem *)ioremap(HI_REG_BASE_L2CACHE, sizeof(void *));
#endif
}
#endif
