...
STATIC void bfgx_enable_gpio_wkup_dev_complete(struct ps_core_s *ps_core_d);
#endif
...
#ifdef _PRE_H2D_GPIO_WKUP
    bfgx_thread_msg_handle,      // SYS_INF_ENABLE_GPIO_WKUP_BFG_RSP = 0x22
#endif
};
...
STATIC void bfgx_thread_msg_handle(struct ps_core_s *ps_core_d, uint8_t *buf_ptr)
{
    ...
    switch (msg) {
        ...
        case SYS_INF_GNSS_LPPE_EXIT:
            ...
            break;
#ifdef _PRE_H2D_GPIO_WKUP
        case SYS_INF_ENABLE_GPIO_WKUP_BFG_RSP:
            bfgx_enable_gpio_wkup_dev_complete(ps_core_d);
            break;
#endif
        default:
            break;
...
STATIC void bfgx_recv_device_chr(struct ps_core_s *ps_core_d, uint8_t *buf_ptr)
{
    ...
    chr_dev_exception_callback((uint8_t *)pst_chr, (pst_chr->errlen + sizeof(chr_errno_with_arg_stru)));
#else
...
STATIC void bfgx_recv_heartbeat(struct ps_core_s *ps_core_d, uint8_t *buf_ptr)
{
    uint8_t syschar;
...
STATIC void bfgx_enable_gpio_wkup_dev_complete(struct ps_core_s *ps_core_d)
{
    ...
    if (pm_data != NULL) {
        complete(&pm_data->gpio_wkup_dev_ack_comp);
    }
    return;
}
#endif
