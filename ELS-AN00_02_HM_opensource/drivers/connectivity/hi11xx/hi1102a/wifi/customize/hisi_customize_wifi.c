...
oal_int32 hwifi_custom_host_read_dyn_cali_nvram(oal_void)
{
    ...
    for (idx = WLAN_CFG_DTS_NVRAM_RATIO_PA2GCCKA0; idx < WLAN_CFG_DTS_NVRAM_END; idx++) {
        ...
        ret = memcpy_s(puc_cust_nvram_info + (idx * DY_CALI_PARAMS_LEN * OAL_SIZEOF(oal_uint8)),
                       (WLAN_CFG_DTS_NVRAM_END - idx) * DY_CALI_PARAMS_LEN,
    ...
    oal_uint8 uc_band_id, oal_uint32 ul_nvram_params)
{
    ...
    ret = memcpy_s(nvram_params, OAL_SIZEOF(nvram_params),
    ...
    for (uc_cfg_id = 0; uc_cfg_id < CUS_NUM_OF_SAR_PER_BAND_PAR_NUM; uc_cfg_id++) {
        for (uc_band_id = 0; uc_band_id < CUS_NUM_OF_SAR_PARAMS; uc_band_id++) {
            ...
            if (l_ret != INI_SUCC) {
                ...
                /* 读取失败时,使用初始值 */
                ul_nvram_params = g_nvram_init_params[ul_cus_id];
            }
...
oal_void hwifi_config_hitalk_base_power(oal_int32 l_plat_tag)
{
    ...
    if ((l_ret == INI_FAILED) || (l_base_power_tmp > MAX_TXPOWER_MAX) || (l_base_power_tmp < MAX_TXPOWER_MIN)) {
        /* 读取失败就使用系统默认值 */
        g_cust_nv_params.uc_nb_base_power = g_nvram_init_params[NVRAM_PARAMS_TX_BASE_POWER_2P4G_NB];
    } else {
        g_cust_nv_params.uc_nb_base_power = l_base_power_tmp;
    }
}
...
oal_int32 hwifi_config_init_base_power(oal_void)
{
    ...
    for (l_cfg_id = NVRAM_PARAMS_TX_BASE_POWER_START; l_cfg_id <= NVRAM_PARAMS_TX_BASE_POWER_END; l_cfg_id++) {
        ...
        if ((l_ret == INI_FAILED) || (l_base_power_tmp > MAX_TXPOWER_MAX) || (l_base_power_tmp < MAX_TXPOWER_MIN)) {
            /* 读取失败就使用系统默认值 */
            auc_base_power[l_base_power_idx] = g_nvram_init_params[l_cfg_id];
        } else {
...
OAL_STATIC int32 hwifi_config_init_private_custom(void)
{
    ...
    for (l_cfg_id = 0; l_cfg_id < WLAN_CFG_PRIV_BUTT; l_cfg_id++) {
        ...
        if (l_ret == INI_FAILED) {
            g_priv_cust_params[l_cfg_id].l_val = g_priv_cust_init_params[l_cfg_id];
    ...
    if (ul_ret == OAL_FAIL) {
        ...
        for (uc_cali_param_idx = DY_CALI_PARAMS_BASE_NUM + 1; uc_cali_param_idx < DY_CALI_PARAMS_NUM_WLAN - 1;
            ...
            cus_flush_nv_ratio_by_delt_pow(g_pro_line_params[uc_cali_param_idx].l_pow_par2,
                                           g_pro_line_params[uc_cali_param_idx].l_pow_par1,
                                           g_pro_line_params[uc_cali_param_idx].l_pow_par0,
...
int32 hwifi_get_mac_addr(uint8 *puc_buf)
{
    ...
    if ((g_wifimac[0] != 0) || (g_wifimac[1] != 0) || (g_wifimac[2] != 0) || (g_wifimac[3] != 0) ||
        (g_wifimac[4] != 0) || (g_wifimac[5] != 0)) {
...
#ifdef CONFIG_ARCH_PLATFORM
    ret = nve_direct_access_interface(&st_info);
#else
    ret = hisi_nve_direct_access(&st_info);
#endif
    if (!ret) {
