                                               ...
                                               oal_netbuf_head_stru *pst_netbuf_head)
{
    ...
    if (oal_unlikely((pst_event_hdr == OAL_PTR_NULL) || (pst_netbuf_head == OAL_PTR_NULL))) {
        oam_error_log2(0, OAM_SF_RX, "{hmac_rx_transmit_to_wlan::param null, %x %x.}", (uintptr_t)pst_event_hdr,
                       (uintptr_t)pst_netbuf_head);
...
#ifndef WIN32
    hmac_rx_ctl_stru *pst_rx_ctrl = OAL_PTR_NULL; /* 指向MPDU控制块信息的指针 */
    oal_netbuf_stru *pst_mac_llc_snap_netbuf = OAL_PTR_NULL;
