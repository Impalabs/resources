...
OAL_STATIC oal_void hmac_thread_bind_fast_cpu(oal_void)
{
    ...
    for (uc_cpuid = 0; uc_cpuid < OAL_BUS_MAXCPU_NUM; uc_cpuid++) {
        ...
        lpcpu_cpufreq_update_req(&g_ast_cpufreq[uc_cpuid], g_ast_cpumaxfreq[uc_cpuid].max_cpu_freq);
#else
...
#ifdef CONFIG_ARCH_PLATFORM
        lpcpu_cpufreq_update_req(&g_ast_cpufreq[uc_cpuid], 0);
#else
...
OAL_STATIC oal_uint32 hmac_wifi_calculate_stat(wifi_txrx_calculated_stat_stru *pst_calculated_stat)
{
    ...
    pst_calculated_stat->ul_tx_large_pps = (oal_atomic_read(&pst_pkgs_info->ul_tx_large_pkt) * 1000) / /* 1s等于1000ms */
                                                 ul_dur_ms;
    pst_calculated_stat->ul_rx_large_pps = (oal_atomic_read(&pst_pkgs_info->ul_rx_large_pkt) * 1000) / /* 1s等于1000ms */
                                                 ul_dur_ms;
    pst_calculated_stat->ul_tx_small_pps = (oal_atomic_read(&pst_pkgs_info->ul_tx_small_pkt) * 1000) / /* 1s等于1000ms */
                                                 ul_dur_ms;
    pst_calculated_stat->ul_rx_small_pps = (oal_atomic_read(&pst_pkgs_info->ul_rx_small_pkt) * 1000) / /* 1s等于1000ms */
                                                 ul_dur_ms;
