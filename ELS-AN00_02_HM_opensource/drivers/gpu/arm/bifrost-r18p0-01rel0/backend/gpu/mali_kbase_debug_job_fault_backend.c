		...
		int reg_range)
{
	...
	for (i = 0; i < sizeof(gpu_control_reg_snapshot)/4; i++) { //lint !e574
	...
	for (i = 0; i < sizeof(job_control_reg_snapshot)/4; i++) { //lint !e574
		kctx->reg_dump[offset] =
	...
	for (j = 0; j < slot_number; j++)	{
		for (i = 0; i < sizeof(job_slot_reg_snapshot)/4; i++) { //lint !e574
			kctx->reg_dump[offset] =
	...
	for (i = 0; i < sizeof(mmu_reg_snapshot)/4; i++) { //lint !e574
	...
	for (j = 0; j < as_number; j++) {
		for (i = 0; i < sizeof(as_reg_snapshot)/4; i++) { //lint !e574
			kctx->reg_dump[offset] =
...
bool kbase_job_fault_get_reg_snapshot(struct kbase_context *kctx)
{
	...
	while (kctx->reg_dump[offset] != REGISTER_DUMP_TERMINATION_FLAG) {
		kctx->reg_dump[offset+1] = //lint !e679
