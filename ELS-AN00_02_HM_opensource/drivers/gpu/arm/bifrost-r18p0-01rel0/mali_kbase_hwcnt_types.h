	...
	size_t blk_inst)
{
	...
	memset(block_enable_map, 0, bitfld_cnt * KBASE_HWCNT_BITFIELD_BYTES); /* unsafe_function_ignore: memset */
}
...
static inline void kbase_hwcnt_enable_map_disable_all(
	struct kbase_hwcnt_enable_map *dst)
{
	memset(dst->enable_map, 0, dst->metadata->enable_map_bytes); /* unsafe_function_ignore: memset */
}
...
{
	memcpy(dst->enable_map, /* unsafe_function_ignore: memcpy */
	       src->enable_map,
...
static inline void kbase_hwcnt_dump_buffer_block_zero(
	...
	size_t val_cnt)
{
	memset(dst_blk, 0, (val_cnt * KBASE_HWCNT_VALUE_BYTES)); /* unsafe_function_ignore: memset */
}
	...
	size_t val_cnt)
{
	 ...
	 */
	memcpy(dst_blk, src_blk, (val_cnt * KBASE_HWCNT_VALUE_BYTES)); /* unsafe_function_ignore: memcpy */
}
	...
	size_t ctr_cnt)
{
	 ...
	 */
	memcpy(dst_blk, src_blk, hdr_cnt * KBASE_HWCNT_VALUE_BYTES); /* unsafe_function_ignore: memcpy */
