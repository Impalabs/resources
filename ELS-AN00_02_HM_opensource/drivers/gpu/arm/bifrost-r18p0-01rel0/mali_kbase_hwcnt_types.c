...
void kbase_hwcnt_dump_buffer_free(struct kbase_hwcnt_dump_buffer *dump_buf)
{
	...
	memset(dump_buf, 0, sizeof(*dump_buf)); /* unsafe_function_ignore: memset */
}
	...
	struct kbase_hwcnt_dump_buffer_array *dump_bufs)
{
	...
	memset(dump_bufs, 0, sizeof(*dump_bufs)); /* unsafe_function_ignore: memset */
}
	...
	struct kbase_hwcnt_dump_buffer *dst)
{
	...
	memset(dst->dump_buf, 0, dst->metadata->dump_buf_bytes); /* unsafe_function_ignore: memset */
}
	...
	const struct kbase_hwcnt_enable_map *dst_enable_map)
{
	...
	kbase_hwcnt_metadata_for_each_block(metadata, grp, blk, blk_inst) {
		...
		ctr_cnt = KBASE_HWCNT_ALIGN_UPWARDS(hdr_cnt + ctr_cnt,
			(KBASE_HWCNT_BLOCK_BYTE_ALIGNMENT /
			 KBASE_HWCNT_VALUE_BYTES) - hdr_cnt); //lint !e665
