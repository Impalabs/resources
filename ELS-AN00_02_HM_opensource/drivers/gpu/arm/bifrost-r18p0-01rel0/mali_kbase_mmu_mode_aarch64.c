...
static u64 get_mmu_flags(unsigned long flags)
{
	...
	/* Set last buffer flags */
	mmu_flags |= (flags & ENTRY_LB_BITS);
...
static void entry_set_ate(struct kbase_device *kbdev, u64 *entry,
		...
		unsigned int level)
{
	// Encode extra info into page table, such as last buffer info.
	if (kbdev->mgm_dev->ops.gpu_map_page)
		flags |= kbdev->mgm_dev->ops.gpu_map_page(level, as_phys_addr_t(phy));
