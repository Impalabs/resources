	...
	struct fp_data *fp_data)
{
	...
	if (of_property_read_u32(np, "fingerprint,is_pc_product",
		(unsigned int *)(&fp_data->is_pc_product))) {
		fp_data->is_pc_product = 0;
		hwlog_info("%s failed to get is_pc_product\n", __func__);
	}
	return FP_RETURN_SUCCESS;
}
...
{
	hwlog_info("%s\n", __func__);
	struct  fp_data *fp_data = dev_get_drvdata(dev);
	if (fp_data->is_pc_product)
		fp_ioc_enable_irq(fp_data);
	return FP_RETURN_SUCCESS;
}
