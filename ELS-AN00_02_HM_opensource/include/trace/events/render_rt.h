 ...
 * render_rt.h
 *
 * enable related thread alogrithm
 *
...
TRACE_EVENT(add_waker_to_render_rthread,
	...
	TP_printk("waker_comm=%s waker_pid=%d wakee_comm=%s wakee_pid=%d waker_index=%d wakee_index=%d waker_cnt=%d",
		  __entry->waker_comm, __entry->waker_pid, __entry->wakee_comm, __entry->wakee_pid,
		  __entry->waker_index, __entry->wakee_index, __entry->waker_cnt)
);
